...
+ (NSString *)specialdDayFromDate:(NSDate *)date {
    ...
    return [self specialdDayFromYear:components.year month:components.month day:components.day];
}
...
#pragma mark 农历节日 & 阳历节日

+ (NSString *)holiDayFromDate:(NSDate *)date {
    NSString *holiday = [self lunarHolidayFromDate:date];
    return holiday.length > 0 ? holiday : [self solarHolidayFromDate:date];
}
...
+ (NSString *)lunarHolidayFromDate:(NSDate *)date {
    ...
    NSString *key_str = [NSString stringWithFormat:@"%ld-%ld",Month,Day];
