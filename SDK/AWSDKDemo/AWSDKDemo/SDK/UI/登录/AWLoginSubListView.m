...
@implementation AWLoginSubListView

+(instancetype)factory_loginSublistWithArr:(NSArray *)arr
{
    
    AWLoginSubListView *looginSubListV = [[AWLoginSubListView alloc]initWithFrame:CGRectMake(0, 0, ViewWidth, ViewHeight-LogoViewHeight)];
    looginSubListV.isChecked = YES;
    [looginSubListV configUIWithArr:arr];
    return looginSubListV;
}
...
//    self.logoview.image = [UIImage imageNamed:@"AWSDK.bundle/logo.png"];
...
-(void)configPortrailUIWithArr:(NSArray *)arr
{
    ...
    [self addProtolLabWithYY:ViewHeight-40+self.addHeight - AdaptWidth(80)];
    AWHGHALLButton *CustomerBtn=  [[AWHGHALLButton alloc]initWithFrame:CGRectMake((ViewWidth-100)/2.0, ViewHeight-40+self.addHeight, 100, 15)];
...
//    AWHGHALLButton *moreLoginBtn=  [[AWHGHALLButton alloc]initWithFrame:CGRectMake((ViewWidth-100)/2.0, ViewHeight-40+self.addHeight, 100, 15)];
//    [moreLoginBtn setTitle:@"更多登录方式" forState:UIControlStateNormal];
//    [moreLoginBtn setTitleColor:RGBA(255, 122, 15, 1) forState:UIControlStateNormal];
//    moreLoginBtn.titleLabel.font = FONTSIZE(12);
//    [moreLoginBtn addTarget:self action:@selector(moreLogin) forControlEvents:UIControlEventTouchUpInside];
    UIView *lineview = [[UIView alloc]initWithFrame:CGRectMake(MarginX, ViewHeight - 85  +self.addHeight-AdaptWidth(100), ViewWidth-2*MarginX, 1)];
    lineview.backgroundColor = RGBA(200, 200, 200, 1);
//    [self.mainView addSubview:lineview];
    
    GGGLabel *noticeLab = [[GGGLabel alloc]initWithFrame:CGRectMake(MarginX, ViewHeight-70+self.addHeight-AdaptWidth(80)-AdaptWidth(20), TFWidth, 35)];
...
-(void)setArr:(NSArray *)arr
{
    ...
    NSArray *containArr = @[LOGIN_PHONE,LOGIN_VISITOR,LOGIN_FACEBOOK,LOGIN_GOOGLE];
...
//        loginBtn.backgroundColor = [UIColor redColor];
    ...
    [self addProtolLabWithYY:AdaptWidth(165)-AdaptWidth(20)];
...
//        frame.size.height = frame.size.height - 20;
    ...
    UIView *lineview = [[UIView alloc]initWithFrame:CGRectMake(MarginX, ViewHeight - 85 + 15  +self.addHeight-AdaptWidth(100), ViewWidth-2*MarginX, 1)];
    lineview.backgroundColor = RGBA(200, 200, 200, 1);
//    [self.mainView addSubview:lineview];
    
    GGGLabel *noticeLab = [[GGGLabel alloc]initWithFrame:CGRectMake(MarginX, ViewHeight-55+self.addHeight-AdaptWidth(100)-AdaptWidth(15), TFWidth, 35)];
    noticeLab.numberOfLines = 0;
    [self.mainView addSubview:noticeLab];
    NSString *appendStr = [NSString stringWithFormat:@"%@%@",GUOJIHUA(@"注意："),GUOJIHUA(@"不同登录方式的账号是独立的")];
    NSMutableAttributedString *attr = [[NSMutableAttributedString alloc]initWithString:appendStr];
