...
@implementation GGGTextSimpleMarkdownParser {
    UIFont *_font;
...
- (void)_updateFonts {
    ...
    _boldFont = GGGTextFontWithBold(_font);
    _italicFont = GGGTextFontWithItalic(_font);
    _boldItalicFont = GGGTextFontWithBoldItalic(_font);
...
- (void)setColorWithBrightTheme {
    ...
    _border = [GGGTextBorder new];
    _border.lineStyle = GGGTextLineStyleSingle;
    ...
    _border.strokeWidth = GGGTextCGFloatFromPixel(1);
}
...
- (BOOL)parseText:(NSMutableAttributedString *)text selectedRange:(NSRangePointer)range {
    ...
    [_regexUnderline enumerateMatchesInString:str options:0 range:NSMakeRange(0, str.length) usingBlock:^(NSTextCheckingResult *result, NSMatchingFlags flags, BOOL *stop) {
        ...
        [text yy_setColor:_controlTextColor range:NSMakeRange(r.location, 2)];
        [text yy_setColor:_controlTextColor range:NSMakeRange(r.location + r.length - 2, 2)];
        [text yy_setTextUnderline:[GGGTextDecoration decorationWithStyle:GGGTextLineStyleSingle width:@1 color:nil] range:NSMakeRange(r.location + 2, r.length - 4)];
    }];
    ...
    [_regexStrikethrough enumerateMatchesInString:str options:0 range:NSMakeRange(0, str.length) usingBlock:^(NSTextCheckingResult *result, NSMatchingFlags flags, BOOL *stop) {
        ...
        [text yy_setColor:_controlTextColor range:NSMakeRange(r.location, 2)];
        [text yy_setColor:_controlTextColor range:NSMakeRange(r.location + r.length - 2, 2)];
        [text yy_setTextStrikethrough:[GGGTextDecoration decorationWithStyle:GGGTextLineStyleSingle width:@1 color:nil] range:NSMakeRange(r.location + 2, r.length - 4)];
    }];
    ...
    [_regexCodeBlock enumerateMatchesInString:str options:0 range:NSMakeRange(0, str.length) usingBlock:^(NSTextCheckingResult *result, NSMatchingFlags flags, BOOL *stop) {
        ...
        if (lenStart + lenEnd < r.length) {
            ...
            border.lineStyle = GGGTextLineStyleSingle;
            ...
            border.strokeWidth = GGGTextCGFloatFromPixel(2);
...
@implementation GGGTextSimpleEmoticonParser {
...
- (BOOL)parseText:(NSMutableAttributedString *)text selectedRange:(NSRangePointer)range {
    ...
    for (NSUInteger i = 0, max = matches.count; i < max; i++) {
        ...
        [atr yy_setTextBackedString:[GGGTextBackedString stringWithString:subStr] range:NSMakeRange(0, atr.length)];
