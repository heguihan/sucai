...
#import "GGGTextContainerView.h"
#import "GGGTextSelectionView.h"
#import "GGGTextMagnifier.h"
#import "GGGTextEffectWindow.h"
...
#define kSystemVersion _GGGDeviceSystemVersion()
#endif
...
NSString *const GGGTextViewTextDidBeginEditingNotification = @"GGGTextViewTextDidBeginEditing";
NSString *const GGGTextViewTextDidChangeNotification = @"GGGTextViewTextDidChange";
NSString *const GGGTextViewTextDidEndEditingNotification = @"GGGTextViewTextDidEndEditing";


typedef NS_ENUM (NSUInteger, GGGTextGrabberDirection) {
    kStart = 1,
    kEnd   = 2,
};

typedef NS_ENUM(NSUInteger, GGGTextMoveDirection) {
    kLeft   = 1,
    kTop    = 2,
    kRight  = 3,
    kBottom = 4,
...
@interface _GGGTextViewUndoObject : NSObject
@property (nonatomic, strong) NSAttributedString *text;
@property (nonatomic, assign) NSRange selectedRange;
@end
@implementation _GGGTextViewUndoObject
+ (instancetype)objectWithText:(NSAttributedString *)text range:(NSRange)range {
    _GGGTextViewUndoObject *obj = [self new];
...
@interface GGGTextView () <UIScrollViewDelegate, UIAlertViewDelegate, GGGTextDebugTarget, GGGTextKeyboardObserver> {
    
    GGGTextRange *_selectedTextRange; /// nonnull
    GGGTextRange *_markedTextRange;
    
    __weak id<GGGTextViewDelegate> _outerDelegate;
    ...
    GGGTextContainer *_innerContainer; ///< nonnull, inner text container
    GGGTextLayout *_innerLayout; ///< inner text layout, the text in this layout is longer than `_innerText` by appending '\n'
    
    GGGTextContainerView *_containerView; ///< nonnull
    GGGTextSelectionView *_selectionView; ///< nonnull
    GGGTextMagnifier *_magnifierCaret; ///< nonnull
    GGGTextMagnifier *_magnifierRanged; ///< nonnull
    ...
    GGGTextRange *_trackingRange; ///< the range in _innerLayout, may out of _innerText.
    ...
    struct {
        unsigned int trackingGrabber : 2;       ///< GGGTextGrabberDirection, current tracking grabber
        ...
        unsigned int touchMoved : 3;            ///< GGGTextMoveDirection, move direction after touch began
...
@synthesize selectedTextRange = _selectedTextRange;  //copy nonnull (GGGTextRange*)
@synthesize markedTextRange = _markedTextRange;      //readonly     (GGGTextRange*)
...
- (void)_commitUpdate {
    ...
    [[GGGTextTransaction transactionWithTarget:self selector:@selector(_updateIfNeeded)] commit];
#else
    [self _update];
#endif
}
...
- (void)_updateLayout {
    ...
    [text removeAttribute:GGGTextBorderAttributeName range:NSMakeRange(_innerText.length, 1)];
    [text removeAttribute:GGGTextBackgroundBorderAttributeName range:NSMakeRange(_innerText.length, 1)];
    ...
    _innerLayout = [GGGTextLayout layoutWithContainer:_innerContainer text:text];
...
- (void)_updateSelectionView {
    ...
    [[GGGTextEffectWindow sharedWindow] hideSelectionDot:_selectionView];
    if (!_innerLayout) return;
    
    NSMutableArray *allRects = [NSMutableArray new];
    BOOL containsDot = NO;
    
    GGGTextRange *selectedRange = _selectedTextRange;
    ...
    [allRects enumerateObjectsUsingBlock:^(GGGTextSelectionRect *rect, NSUInteger idx, BOOL *stop) {
    ...
    if (!_state.firstShowDot && containsDot) {
        ...
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.02 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            [[GGGTextEffectWindow sharedWindow] showSelectionDot:_selectionView];
        });
    }
    [[GGGTextEffectWindow sharedWindow] showSelectionDot:_selectionView];
...
- (void)_commitPlaceholderUpdate {
    ...
    [[GGGTextTransaction transactionWithTarget:self selector:@selector(_updatePlaceholderIfNeeded)] commit];
#else
...
- (void)_updatePlaceholder {
    ...
    if (_placeholderAttributedText.length > 0) {
        ...
        container.truncationType = GGGTextTruncationTypeEnd;
        container.truncationToken = nil;
        GGGTextLayout *layout = [GGGTextLayout layoutWithContainer:container text:_placeholderAttributedText];
...
- (void)_updateTextRangeByTrackingCaret {
    ...
    GGGTextPosition *newPos = [_innerLayout closestPositionToPoint:trackingPoint];
    if (newPos) {
        ...
        GGGTextRange *newRange = [GGGTextRange rangeWithRange:NSMakeRange(newPos.offset, 0) affinity:newPos.affinity];
...
- (void)_updateTextRangeByTrackingGrabber {
    ...
    GGGTextPosition *position = [_innerLayout positionForPoint:magPoint
    ...
    if (position) {
        ...
        if ((NSUInteger)position.offset > _innerText.length) {
            position = [GGGTextPosition positionWithOffset:_innerText.length];
        }
        GGGTextRange *newRange = [GGGTextRange rangeWithStart:(isStart ? position : _selectedTextRange.start)
...
- (void)_showMagnifierCaret {
    ...
    if (!_state.showingMagnifierCaret) {
        ...
        [[GGGTextEffectWindow sharedWindow] showMagnifier:_magnifierCaret];
    } else {
        [[GGGTextEffectWindow sharedWindow] moveMagnifier:_magnifierCaret];
    }
}
...
- (void)_showMagnifierRanged {
    ...
    if (_state.showingMagnifierCaret) {
        ...
        [[GGGTextEffectWindow sharedWindow] hideMagnifier:_magnifierCaret];
    }
    ...
    if (lineIndex < _innerLayout.lines.count) {
        GGGTextLine *line = _innerLayout.lines[lineIndex];
        CGRect lineRect = [self _convertRectFromLayout:line.bounds];
        if (_verticalForm) {
            magPoint.x = GGGTEXT_CLAMP(magPoint.x, CGRectGetMinX(lineRect), CGRectGetMaxX(lineRect));
        } else {
            magPoint.y = GGGTEXT_CLAMP(magPoint.y, CGRectGetMinY(lineRect), CGRectGetMaxY(lineRect));
        }
        ...
        if (!_state.showingMagnifierRanged) {
            ...
            [[GGGTextEffectWindow sharedWindow] showMagnifier:_magnifierRanged];
        } else {
            [[GGGTextEffectWindow sharedWindow] moveMagnifier:_magnifierRanged];
        }
    }
}
...
- (void)_showMenu {
    ...
    if (_selectionView.caretVisible) {
        ...
        GGGTextSelectionRect *sRect = _selectionView.selectionRects.firstObject;
        rect = sRect.rect;
        ...
        GGGTextKeyboardManager *mgr = [GGGTextKeyboardManager defaultManager];
...
- (void)_scrollRangeToVisible:(GGGTextRange *)range {
    if (!range) return;
...
- (void)_keyboardChanged {
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        if ([GGGTextKeyboardManager defaultManager].keyboardVisible) {
            [self _scrollRangeToVisible:_selectedTextRange];
        } else {
...
- (void)_trackDidLongPress {
    ...
    if (_state.showingHighlight) {
        ...
        if (_highlight.longPressAction) {
            ...
            CGRect rect = [_innerLayout rectForRange:[GGGTextRange rangeWithRange:_highlightRange]];
            rect = [self _convertRectFromLayout:rect];
        ...
        if (_state.trackingTouch) {
            if (_state.trackingGrabber) {
                ...
                if (newPos) {
                    ...
                    _trackingRange = [GGGTextRange rangeWithRange:NSMakeRange(newPos.offset, 0) affinity:newPos.affinity];
                    [self _updateSelectionView];
                }
                [self _hideMenu];
...
- (void)_fixSelectionDot {
    if (GGGTextIsAppExtension()) return;
    CGPoint origin = [self yy_convertPoint:CGPointZero toViewOrWindow:[GGGTextEffectWindow sharedWindow]];
...
- (GGGTextRange *)_getClosestTokenRangeAtPosition:(GGGTextPosition *)position {
    position = [self _correctedTextPosition:position];
    if (!position) return nil;
    GGGTextRange *range = nil;
    if (_tokenizer) {
    ...
    if (range.asRange.length == 0) {
        ...
        GGGTextRange *extStart = [_innerLayout textRangeByExtendingPosition:range.start];
        GGGTextRange *extEnd = [_innerLayout textRangeByExtendingPosition:range.end];
        if (extStart && extEnd) {
            NSArray *arr = [@[extStart.start, extStart.end, extEnd.start, extEnd.end] sortedArrayUsingSelector:@selector(compare:)];
            range = [GGGTextRange rangeWithStart:arr.firstObject end:arr.lastObject];
        }
    }
    
    range = [self _correctedTextRange:range];
    if (range.asRange.length == 0) {
        range = [GGGTextRange rangeWithRange:NSMakeRange(0, _innerText.length)];
    }
...
/// Try to get the character range/position with word granularity from the tokenizer.
- (GGGTextRange *)_getClosestTokenRangeAtPoint:(CGPoint)point {
    point = [self _convertPointToLayout:point];
    GGGTextRange *touchRange = [_innerLayout closestTextRangeAtPoint:point];
    touchRange = [self _correctedTextRange:touchRange];
    
    if (_tokenizer && touchRange) {
        GGGTextRange *encEnd = (id)[_tokenizer rangeEnclosingPosition:touchRange.end withGranularity:UITextGranularityWord inDirection:UITextStorageDirectionBackward];
        GGGTextRange *encStart = (id)[_tokenizer rangeEnclosingPosition:touchRange.start withGranularity:UITextGranularityWord inDirection:UITextStorageDirectionForward];
        if (encEnd && encStart) {
            NSArray *arr = [@[encEnd.start, encEnd.end, encStart.start, encStart.end] sortedArrayUsingSelector:@selector(compare:)];
            touchRange = [GGGTextRange rangeWithStart:arr.firstObject end:arr.lastObject];
        }
    }
    
    if (touchRange) {
        GGGTextRange *extStart = [_innerLayout textRangeByExtendingPosition:touchRange.start];
        GGGTextRange *extEnd = [_innerLayout textRangeByExtendingPosition:touchRange.end];
        if (extStart && extEnd) {
            NSArray *arr = [@[extStart.start, extStart.end, extEnd.start, extEnd.end] sortedArrayUsingSelector:@selector(compare:)];
            touchRange = [GGGTextRange rangeWithStart:arr.firstObject end:arr.lastObject];
        }
    ...
    if (!touchRange) touchRange = [GGGTextRange defaultRange];
    
    if (_innerText.length && touchRange.asRange.length == 0) {
        touchRange = [GGGTextRange rangeWithRange:NSMakeRange(0, _innerText.length)];
    }
...
- (GGGTextHighlight *)_getHighlightAtPoint:(CGPoint)point range:(NSRangePointer)range {
    ...
    point = [self _convertPointToLayout:point];
    GGGTextRange *textRange = [_innerLayout textRangeAtPoint:point];
    ...
    GGGTextHighlight *highlight = [text attribute:GGGTextHighlightAttributeName
                                         atIndex:startIndex
...
- (CGFloat)_getMagnifierRangedOffset {
    ...
    GGGTextPosition *position = [_innerLayout closestPositionToPoint:magPoint];
    NSUInteger lineIndex = [_innerLayout lineIndexForPosition:position];
    if (lineIndex < _innerLayout.lines.count) {
        GGGTextLine *line = _innerLayout.lines[lineIndex];
        if (_verticalForm) {
            magPoint.x = GGGTEXT_CLAMP(magPoint.x, line.left, line.right);
            return magPoint.x - line.position.x + kMagnifierRangedPopoverOffset;
        } else {
            magPoint.y = GGGTEXT_CLAMP(magPoint.y, line.top, line.bottom);
...
/// Return a GGGTextMoveDirection from `_touchBeganPoint` to `_trackingPoint`.
...
- (BOOL)_isTextPositionValid:(GGGTextPosition *)position {
    if (!position) return NO;
    if (position.offset < 0) return NO;
    if (position.offset > _innerText.length) return NO;
    if (position.offset == 0 && position.affinity == GGGTextAffinityBackward) return NO;
    if (position.offset == _innerText.length && position.affinity == GGGTextAffinityBackward) return NO;
    return YES;
}

/// Whether the range is valid (not out of bounds).
- (BOOL)_isTextRangeValid:(GGGTextRange *)range {
...
- (GGGTextPosition *)_correctedTextPosition:(GGGTextPosition *)position {
    if (!position) return nil;
    if ([self _isTextPositionValid:position]) return position;
    if (position.offset < 0) {
        return [GGGTextPosition positionWithOffset:0];
    }
    if (position.offset > _innerText.length) {
        return [GGGTextPosition positionWithOffset:_innerText.length];
    }
    if (position.offset == 0 && position.affinity == GGGTextAffinityBackward) {
        return [GGGTextPosition positionWithOffset:position.offset];
    }
    if (position.offset == _innerText.length && position.affinity == GGGTextAffinityBackward) {
        return [GGGTextPosition positionWithOffset:position.offset];
    }
    return position;
}
...
- (GGGTextRange *)_correctedTextRange:(GGGTextRange *)range {
    if (!range) return nil;
    if ([self _isTextRangeValid:range]) return range;
    GGGTextPosition *start = [self _correctedTextPosition:range.start];
    GGGTextPosition *end = [self _correctedTextPosition:range.end];
    return [GGGTextRange rangeWithStart:start end:end];
}
...
- (void)_replaceRange:(GGGTextRange *)range withText:(NSString *)text notifyToDelegate:(BOOL)notify{
    if (NSEqualRanges(range.asRange, _selectedTextRange.asRange)) {
        ...
        _selectedTextRange = [GGGTextRange rangeWithRange:newRange];
        ...
        if (range.asRange.length != text.length) {
            ...
            if (unionRange.length == 0) {
                ...
                _selectedTextRange = [GGGTextRange rangeWithRange:NSMakeRange(range.start.offset + text.length, 0)];
...
- (void)_updateAttributesHolder {
    if (_innerText.length > 0) {
        ...
        [_typingAttributesHolder removeAttribute:GGGTextBorderAttributeName range:NSMakeRange(0, _typingAttributesHolder.length)];
        [_typingAttributesHolder removeAttribute:GGGTextBackgroundBorderAttributeName range:NSMakeRange(0, _typingAttributesHolder.length)];
    }
}
...
- (BOOL)_parseText {
    if (self.textParser) {
        GGGTextRange *oldTextRange = _selectedTextRange;
        NSRange newRange = _selectedTextRange.asRange;
        ...
        GGGTextRange *newTextRange = [GGGTextRange rangeWithRange:newRange];
        ...
        switch (result.resultType) {
            ...
            case NSTextCheckingTypePhoneNumber: {
                ...
                if (_highlightTextAttributes.count) {
                    GGGTextHighlight *highlight = [GGGTextHighlight highlightWithAttributes:_highlightTextAttributes];
...
- (UIViewController *)_getRootViewController {
    ...
    UIApplication *app = GGGTextSharedApplication();
...
- (void)_resetUndoAndRedoStack {
    ...
    _GGGTextViewUndoObject *object = [_GGGTextViewUndoObject objectWithText:_innerText.copy range:_selectedTextRange.asRange];
...
- (void)_saveToUndoStack {
    ...
    _GGGTextViewUndoObject *lastObject = _undoStack.lastObject;
...
- (void)_saveToRedoStack {
    if (!_allowsUndoAndRedo) return;
    _GGGTextViewUndoObject *lastObject = _redoStack.lastObject;
    if ([lastObject.text isEqualToAttributedString:self.attributedText]) return;
...
- (BOOL)_canUndo {
    ...
    _GGGTextViewUndoObject *object = _undoStack.lastObject;
...
- (void)_redo {
    ...
    _GGGTextViewUndoObject *object = _redoStack.lastObject;
...
- (void)_setTextParser:(id<GGGTextParser>)textParser {
...
- (void)_setLinePositionModifier:(id<GGGTextLinePositionModifier>)linePositionModifier {
...
- (void)_initTextView {
    ...
    // UITextInput
    _selectedTextRange = [GGGTextRange defaultRange];
    ...
    GGGTextHighlight *highlight = [GGGTextHighlight new];
    GGGTextBorder * border = [GGGTextBorder new];
    ...
    _containerView = [GGGTextContainerView new];
    _containerView.hostView = self;
    
    _selectionView = [GGGTextSelectionView new];
    _selectionView.userInteractionEnabled = NO;
    _selectionView.hostView = self;
    _selectionView.color = [self _defaultTintColor];
    
    _magnifierCaret = [GGGTextMagnifier magnifierWithType:GGGTextMagnifierTypeCaret];
    _magnifierCaret.hostView = _containerView;
    _magnifierRanged = [GGGTextMagnifier magnifierWithType:GGGTextMagnifierTypeRanged];
    ...
    self.debugOption = [GGGTextDebugOption sharedDebugOption];
    ...
    [[GGGTextKeyboardManager defaultManager] addObserver:self];
...
- (void)dealloc {
    ...
    [[GGGTextKeyboardManager defaultManager] removeObserver:self];
...
- (void)scrollRangeToVisible:(NSRange)range {
    GGGTextRange *textRange = [GGGTextRange rangeWithRange:range];
    textRange = [self _correctedTextRange:textRange];
...
- (void)setText:(NSString *)text {
    ...
    [self replaceRange:[GGGTextRange rangeWithRange:NSMakeRange(0, _innerText.length)] withText:text];
}
...
- (void)setDataDetectorTypes:(UIDataDetectorTypes)dataDetectorTypes {
    ...
    NSTextCheckingType type = GGGTextNSTextCheckingTypeFromUIDataDetectorType(dataDetectorTypes);
...
- (void)setTextParser:(id<GGGTextParser>)textParser {
    ...
    if (textParser && _text.length) {
        [self replaceRange:[GGGTextRange rangeWithRange:NSMakeRange(0, _text.length)] withText:_text];
    }
    [self _resetUndoAndRedoStack];
    [self _commitUpdate];
}
...
- (void)setAttributedText:(NSAttributedString *)attributedText {
    ...
    if (text.length == 0) {
        [self replaceRange:[GGGTextRange rangeWithRange:NSMakeRange(0, _innerText.length)] withText:@""];
        return;
    }
    ...
    [_inputDelegate textWillChange:self];
     _innerText = text;
    [self _parseText];
    _selectedTextRange = [GGGTextRange rangeWithRange:NSMakeRange(0, _innerText.length)];
    [_inputDelegate textDidChange:self];
    [_inputDelegate selectionDidChange:self];
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:GGGTextViewTextDidChangeNotification object:self];
...
- (void)setSelectedRange:(NSRange)selectedRange {
    ...
    GGGTextRange *range = [GGGTextRange rangeWithRange:selectedRange];
    range = [self _correctedTextRange:range];
    [self _endTouchTracking];
...
- (GGGTextDebugOption *)debugOption {
...
- (void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    if (_state.trackingTouch) {
        ...
        if (_highlight) {
            ...
            GGGTextHighlight *highlight = [self _getHighlightAtPoint:_trackingPoint range:NULL];
            ...
            if (_state.trackingCaret) {
                ...
                if (!_state.touchMoved) {
                    if (_state.selectedWithoutEdit) {
                        ...
                        if (self.isFirstResponder) {
                            GGGTextRange *_oldRange = _trackingRange;
                            [self _updateTextRangeByTrackingCaret];
...
- (BOOL)becomeFirstResponder {
    ...
    if (!isFirstResponder && become) {
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:GGGTextViewTextDidBeginEditingNotification object:self];
    }
    return become;
}
...
- (BOOL)resignFirstResponder {
    ...
    if (resign) {
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:GGGTextViewTextDidEndEditingNotification object:self];
    }
    return resign;
}
...
- (void)paste:(id)sender {
    ...
    if (!atr && _allowsPasteImage) {
        ...
        Class cls = NSClassFromString(@"GGGImage");
        if (cls) {
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wundeclared-selector"
            if (p.yy_GIFData) {
...
#pragma clang diagnostic pop
        ...
        if (!img && p.yy_ImageData) {
            img = [UIImage imageWithData:p.yy_ImageData scale:GGGTextScreenScale()];
        }
        if (img && img.size.width > 1 && img.size.height > 1) {
            id content = img;
            
            if (cls) {
                if ([img conformsToProtocol:NSProtocolFromString(@"GGGAnimatedImage")]) {
                    NSNumber *frameCount = [img valueForKey:@"animatedImageFrameCount"];
                    if (frameCount.integerValue > 1) {
                        Class viewCls = NSClassFromString(@"GGGAnimatedImageView");
    ...
    if (atr) {
        ...
        GGGTextPosition *pos = [self _correctedTextPosition:[GGGTextPosition positionWithOffset:endPosition]];
        GGGTextRange *range = [_innerLayout textRangeByExtendingPosition:pos];
        range = [self _correctedTextRange:range];
        if (range) {
...
- (void)select:(id)sender {
    ...
    GGGTextRange *newRange = [self _getClosestTokenRangeAtPosition:_selectedTextRange.start];
...
- (id<GGGTextViewDelegate>)delegate {
    return _outerDelegate;
}

- (void)setDelegate:(id<GGGTextViewDelegate>)delegate {
...
#pragma mark - @protocol GGGTextKeyboardObserver

- (void)keyboardChangedWithTransition:(GGGTextKeyboardTransition)transition {
    [self _keyboardChanged];
}
...
- (void)deleteBackward {
    ...
    if (!_state.deleteConfirm && range.length == 0 && range.location > 0) {
        ...
        GGGTextBinding *binding = [_innerText attribute:GGGTextBindingAttributeName atIndex:range.location - 1 longestEffectiveRange:&effectiveRange inRange:NSMakeRange(0, _innerText.length)];
        if (binding && binding.deleteConfirm) {
            _state.deleteConfirm = YES;
            [_inputDelegate selectionWillChange:self];
            _selectedTextRange = [GGGTextRange rangeWithRange:effectiveRange];
            _selectedTextRange = [self _correctedTextRange:_selectedTextRange];
            [_inputDelegate selectionDidChange:self];
    ...
    _state.deleteConfirm = NO;
    if (range.length == 0) {
        GGGTextRange *extendRange = [_innerLayout textRangeByExtendingPosition:_selectedTextRange.end inDirection:UITextLayoutDirectionLeft offset:1];
    ...
    [self replaceRange:[GGGTextRange rangeWithRange:range] withText:@""];
}
...
- (void)setSelectedTextRange:(GGGTextRange *)selectedTextRange {
...
- (void)setMarkedText:(NSString *)markedText selectedRange:(NSRange)selectedRange {
    ...
    if (_markedTextRange == nil) {
        _markedTextRange = [GGGTextRange rangeWithRange:NSMakeRange(_selectedTextRange.end.offset, markedText.length)];
        [_innerText replaceCharactersInRange:NSMakeRange(_selectedTextRange.end.offset, 0) withString:markedText];
        _selectedTextRange = [GGGTextRange rangeWithRange:NSMakeRange(_selectedTextRange.start.offset + selectedRange.location, selectedRange.length)];
    } else {
        ...
        _markedTextRange = [GGGTextRange rangeWithRange:NSMakeRange(_markedTextRange.start.offset, markedText.length)];
        _selectedTextRange = [GGGTextRange rangeWithRange:NSMakeRange(_markedTextRange.start.offset + selectedRange.location, selectedRange.length)];
    }
...
- (void)replaceRange:(GGGTextRange *)range withText:(NSString *)text {
    if (!range) return;
    if (!text) text = @"";
...
- (void)setBaseWritingDirection:(UITextWritingDirection)writingDirection forRange:(GGGTextRange *)range {
    if (!range) return;
    range = [self _correctedTextRange:range];
...
- (NSString *)textInRange:(GGGTextRange *)range {
    range = [self _correctedTextRange:range];
    if (!range) return @"";
    return [_innerText.string substringWithRange:range.asRange];
}

- (UITextWritingDirection)baseWritingDirectionForPosition:(GGGTextPosition *)position inDirection:(UITextStorageDirection)direction {
    [self _updateIfNeeded];
    position = [self _correctedTextPosition:position];
...
- (GGGTextPosition *)beginningOfDocument {
    return [GGGTextPosition positionWithOffset:0];
}

- (GGGTextPosition *)endOfDocument {
    return [GGGTextPosition positionWithOffset:_innerText.length];
}

- (GGGTextPosition *)positionFromPosition:(GGGTextPosition *)position offset:(NSInteger)offset {
    ...
    if (newLocation != 0 && newLocation != _innerText.length) {
        // fix emoji
        [self _updateIfNeeded];
        GGGTextRange *extendRange = [_innerLayout textRangeByExtendingPosition:[GGGTextPosition positionWithOffset:newLocation]];
    ...
    GGGTextPosition *p = [GGGTextPosition positionWithOffset:newLocation];
    return [self _correctedTextPosition:p];
}

- (GGGTextPosition *)positionFromPosition:(GGGTextPosition *)position inDirection:(UITextLayoutDirection)direction offset:(NSInteger)offset {
    [self _updateIfNeeded];
    GGGTextRange *range = [_innerLayout textRangeByExtendingPosition:position inDirection:direction offset:offset];
    ...
    GGGTextPosition *newPosition = forward ? range.end : range.start;
    if (newPosition.offset > _innerText.length) {
        newPosition = [GGGTextPosition positionWithOffset:_innerText.length affinity:GGGTextAffinityBackward];
    }
    
    return [self _correctedTextPosition:newPosition];
}

- (GGGTextRange *)textRangeFromPosition:(GGGTextPosition *)fromPosition toPosition:(GGGTextPosition *)toPosition {
    return [GGGTextRange rangeWithStart:fromPosition end:toPosition];
}

- (NSComparisonResult)comparePosition:(GGGTextPosition *)position toPosition:(GGGTextPosition *)other {
    return [position compare:other];
}

- (NSInteger)offsetFromPosition:(GGGTextPosition *)from toPosition:(GGGTextPosition *)toPosition {
    return toPosition.offset - from.offset;
}

- (GGGTextPosition *)positionWithinRange:(GGGTextRange *)range farthestInDirection:(UITextLayoutDirection)direction {
    NSRange nsRange = range.asRange;
    if (direction == UITextLayoutDirectionLeft | direction == UITextLayoutDirectionUp) {
        return [GGGTextPosition positionWithOffset:nsRange.location];
    } else {
        return [GGGTextPosition positionWithOffset:nsRange.location + nsRange.length affinity:GGGTextAffinityBackward];
    }
}

- (GGGTextRange *)characterRangeByExtendingPosition:(GGGTextPosition *)position inDirection:(UITextLayoutDirection)direction {
    [self _updateIfNeeded];
    GGGTextRange *range = [_innerLayout textRangeByExtendingPosition:position inDirection:direction offset:1];
    return [self _correctedTextRange:range];
}
...
- (GGGTextPosition *)closestPositionToPoint:(CGPoint)point {
    [self _updateIfNeeded];
    point = [self _convertPointToLayout:point];
    GGGTextPosition *position = [_innerLayout closestPositionToPoint:point];
    return [self _correctedTextPosition:position];
}

- (GGGTextPosition *)closestPositionToPoint:(CGPoint)point withinRange:(GGGTextRange *)range {
    GGGTextPosition *pos = (id)[self closestPositionToPoint:point];
    if (!pos) return nil;
...
- (GGGTextRange *)characterRangeAtPoint:(CGPoint)point {
    [self _updateIfNeeded];
    point = [self _convertPointToLayout:point];
    GGGTextRange *r = [_innerLayout closestTextRangeAtPoint:point];
...
- (CGRect)caretRectForPosition:(GGGTextPosition *)position {
    ...
    if (!CGRectIsNull(caretRect)) {
        ...
        return GGGTextCGRectPixelRound(caretRect);
    }
    return CGRectZero;
}
...
- (NSArray *)selectionRectsForRange:(GGGTextRange *)range {
    ...
    [rects enumerateObjectsUsingBlock:^(GGGTextSelectionRect *rect, NSUInteger idx, BOOL *stop) {
        rect.rect = [self _convertRectFromLayout:rect.rect];
    }];
    return rects;
}
...
- (UITextStorageDirection)selectionAffinity {
    if (_selectedTextRange.end.affinity == GGGTextAffinityForward) {
...
- (void)setSelectionAffinity:(UITextStorageDirection)selectionAffinity {
    _selectedTextRange = [GGGTextRange rangeWithRange:_selectedTextRange.asRange affinity:selectionAffinity == UITextStorageDirectionForward ? GGGTextAffinityForward : GGGTextAffinityBackward];
    [self _updateSelectionView];
}

- (NSDictionary *)textStylingAtPosition:(GGGTextPosition *)position inDirection:(UITextStorageDirection)direction {
    if (!position) return nil;
...
- (GGGTextPosition *)positionWithinRange:(GGGTextRange *)range atCharacterOffset:(NSInteger)offset {
    if (!range) return nil;
    ...
    else return [GGGTextPosition positionWithOffset:offset];
}

- (NSInteger)characterOffsetOfPosition:(GGGTextPosition *)position withinRange:(GGGTextRange *)range {
...
@interface GGGTextView(IBInspectableProperties)
@end

@implementation GGGTextView(IBInspectableProperties)
