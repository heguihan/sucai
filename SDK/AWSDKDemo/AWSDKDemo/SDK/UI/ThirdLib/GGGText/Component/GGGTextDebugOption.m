...
static GGGTextDebugOption *_sharedDebugOption = nil;
...
void _sharedDebugSetFunction(const void *value, void *context) {
    id<GGGTextDebugTarget> target = (__bridge id<GGGTextDebugTarget>)(value);
...
static void _setSharedDebugOption(GGGTextDebugOption *option) {
    _initSharedDebug();
...
static GGGTextDebugOption *_getSharedDebugOption() {
    _initSharedDebug();
    pthread_mutex_lock(&_sharedDebugLock);
    GGGTextDebugOption *op = _sharedDebugOption;
    pthread_mutex_unlock(&_sharedDebugLock);
    return op;
}

static void _addDebugTarget(id<GGGTextDebugTarget> target) {
    _initSharedDebug();
    pthread_mutex_lock(&_sharedDebugLock);
...
static void _removeDebugTarget(id<GGGTextDebugTarget> target) {
    _initSharedDebug();
    pthread_mutex_lock(&_sharedDebugLock);
...
@implementation GGGTextDebugOption

- (id)copyWithZone:(NSZone *)zone {
    GGGTextDebugOption *op = [self.class new];
...
+ (void)addDebugTarget:(id<GGGTextDebugTarget>)target {
    if (target) _addDebugTarget(target);
}

+ (void)removeDebugTarget:(id<GGGTextDebugTarget>)target {
    if (target) _removeDebugTarget(target);
}

+ (GGGTextDebugOption *)sharedDebugOption {
    return _getSharedDebugOption();
}

+ (void)setSharedDebugOption:(GGGTextDebugOption *)option {
