...
const CGSize GGGTextContainerMaxSize = (CGSize){0x100000, 0x100000};
...
static inline CGSize GGGTextClipCGSize(CGSize size) {
    if (size.width > GGGTextContainerMaxSize.width) size.width = GGGTextContainerMaxSize.width;
    if (size.height > GGGTextContainerMaxSize.height) size.height = GGGTextContainerMaxSize.height;
    return size;
}
 ...
 */
static CGColorRef GGGTextGetCGColor(CGColorRef color) {
...
@implementation GGGTextLinePositionSimpleModifier
- (void)modifyLines:(NSArray *)lines fromText:(NSAttributedString *)text inContainer:(GGGTextContainer *)container {
    if (container.verticalForm) {
        for (NSUInteger i = 0, max = lines.count; i < max; i++) {
            GGGTextLine *line = lines[i];
...
- (id)copyWithZone:(NSZone *)zone {
    GGGTextLinePositionSimpleModifier *one = [self.class new];
...
@implementation GGGTextContainer {
    @package
    BOOL _readonly; ///< used only in GGGTextLayout.implementation
    ...
    GGGTextTruncationType _truncationType;
    NSAttributedString *_truncationToken;
    id<GGGTextLinePositionModifier> _linePositionModifier;
}
...
+ (instancetype)containerWithSize:(CGSize)size insets:(UIEdgeInsets)insets {
    GGGTextContainer *one = [self new];
    one.size = GGGTextClipCGSize(size);
    one.insets = insets;
    return one;
}
...
- (id)copyWithZone:(NSZone *)zone {
    GGGTextContainer *one = [self.class new];
...
reason:@"Cannot change the property of the 'container' in 'GGGTextLayout'." userInfo:nil]; \
return; \
} \
dispatch_semaphore_wait(_lock, DISPATCH_TIME_FOREVER); \
__VA_ARGS__; \
...
- (void)setSize:(CGSize)size {
    Setter(if(!_path) _size = GGGTextClipCGSize(size));
}
...
- (GGGTextTruncationType)truncationType {
    Getter(GGGTextTruncationType type = _truncationType) return type;
}

- (void)setTruncationType:(GGGTextTruncationType)truncationType {
...
- (id<GGGTextLinePositionModifier>)linePositionModifier {
    Getter(id<GGGTextLinePositionModifier> m = _linePositionModifier) return m;
}
...
@interface GGGTextLayout ()

@property (nonatomic, readwrite) GGGTextContainer *container;
...
@property (nonatomic, readwrite) GGGTextLine *truncatedLine;
...
@property (nonatomic, assign) GGGRowEdge *lineRowsEdge; ///< top-left origin

@end



@implementation GGGTextLayout
...
+ (GGGTextLayout *)layoutWithContainerSize:(CGSize)size text:(NSAttributedString *)text {
    GGGTextContainer *container = [GGGTextContainer containerWithSize:size];
    return [self layoutWithContainer:container text:text];
}

+ (GGGTextLayout *)layoutWithContainer:(GGGTextContainer *)container text:(NSAttributedString *)text {
    return [self layoutWithContainer:container text:text range:NSMakeRange(0, text.length)];
}

+ (GGGTextLayout *)layoutWithContainer:(GGGTextContainer *)container text:(NSAttributedString *)text range:(NSRange)range {
    GGGTextLayout *layout = NULL;
    ...
    GGGTextLine *truncatedLine = nil;
    GGGRowEdge *lineRowsEdge = NULL;
    ...
    layout = [[GGGTextLayout alloc] _init];
    layout.text = text;
    ...
    ctFrame = CTFramesetterCreateFrame(ctSetter, GGGTextCFRangeFromNSRange(range), cgPath, (CFTypeRef)frameAttrs);
    ...
    for (NSUInteger i = 0; i < lineCount; i++) {
        ...
        GGGTextLine *line = [GGGTextLine lineWithCTLine:ctLine position:position vertical:isVerticalForm];
    ...
    if (rowCount > 0) {
        if (maximumNumberOfRows > 0) {
            if (rowCount > maximumNumberOfRows) {
                ...
                do {
                    GGGTextLine *line = lines.lastObject;
                    if (!line) break;
        ...
        GGGTextLine *lastLine = lines.lastObject;
        ...
        lineRowsEdge = calloc(rowCount, sizeof(GGGRowEdge));
        ...
        for (NSUInteger i = 0, max = lines.count; i < max; i++) {
            ...
            if ((NSInteger)line.row != lastRowIdx) {
                if (lastRowIdx >= 0) {
                    lineRowsEdge[lastRowIdx] = (GGGRowEdge) {.head = lastHead, .foot = lastFoot };
                }
                lastRowIdx = line.row;
        ...
        for (NSUInteger i = 1; i < rowCount; i++) {
            GGGRowEdge v0 = lineRowsEdge[i - 1];
            GGGRowEdge v1 = lineRowsEdge[i];
    ...
    { // calculate bounding size
        ...
        if (container.path) {
            ...
            rect = UIEdgeInsetsInsetRect(rect,GGGTextUIEdgeInsetsInvert(container.insets));
        }
        rect = CGRectStandardize(rect);
    ...
    visibleRange = GGGTextNSRangeFromCFRange(CTFrameGetVisibleStringRange(ctFrame));
    if (needTruncation) {
        GGGTextLine *lastLine = lines.lastObject;
        ...
        if (container.truncationType != GGGTextTruncationTypeNone) {
            ...
            if (container.truncationToken) {
                ...
                truncationToken = [[NSAttributedString alloc] initWithString:GGGTextTruncationToken attributes:attrs];
                truncationTokenLine = CTLineCreateWithAttributedString((CFAttributedStringRef)truncationToken);
            }
            if (truncationTokenLine) {
                CTLineTruncationType type = kCTLineTruncationEnd;
                if (container.truncationType == GGGTextTruncationTypeStart) {
                    type = kCTLineTruncationStart;
                } else if (container.truncationType == GGGTextTruncationTypeMiddle) {
                ...
                if (ctLastLineExtend) {
                    ...
                    if (ctTruncatedLine) {
                        truncatedLine = [GGGTextLine lineWithCTLine:ctTruncatedLine position:lastLine.position vertical:isVerticalForm];
    ...
    if (isVerticalForm) {
        NSCharacterSet *rotateCharset = GGGTextVerticalFormRotateCharacterSet();
        NSCharacterSet *rotateMoveCharset = GGGTextVerticalFormRotateAndMoveCharacterSet();
        
        void (^lineBlock)(GGGTextLine *) = ^(GGGTextLine *line){
            ...
            for (NSUInteger r = 0; r < runCount; r++) {
                ...
                BOOL isColorGlyph = GGGTextCTFontContainsColorBitmapGlyphs(font);
                
                NSUInteger prevIdx = 0;
                GGGTextRunGlyphDrawMode prevMode = GGGTextRunGlyphDrawModeHorizontal;
                ...
                for (NSUInteger g = 0; g < glyphCount; g++) {
                    ...
                    GGGTextRunGlyphDrawMode mode = glyphRotateMove ? GGGTextRunGlyphDrawModeVerticalRotateMove : (glyphRotate ? GGGTextRunGlyphDrawModeVerticalRotate : GGGTextRunGlyphDrawModeHorizontal);
                    if (g == 0) {
                        prevMode = mode;
                    } else if (mode != prevMode) {
                        GGGTextRunGlyphRange *aRange = [GGGTextRunGlyphRange rangeWithRange:NSMakeRange(prevIdx, g - prevIdx) drawMode:prevMode];
                ...
                if (prevIdx < glyphCount) {
                    GGGTextRunGlyphRange *aRange = [GGGTextRunGlyphRange rangeWithRange:NSMakeRange(prevIdx, glyphCount - prevIdx) drawMode:prevMode];
                    [runRanges addObject:aRange];
                }
                
            }
        };
        for (GGGTextLine *line in lines) {
            lineBlock(line);
        }
    ...
    if (visibleRange.length > 0) {
        ...
        void (^block)(NSDictionary *attrs, NSRange range, BOOL *stop) = ^(NSDictionary *attrs, NSRange range, BOOL *stop) {
            if (attrs[GGGTextHighlightAttributeName]) layout.containsHighlight = YES;
            if (attrs[GGGTextBlockBorderAttributeName]) layout.needDrawBlockBorder = YES;
            if (attrs[GGGTextBackgroundBorderAttributeName]) layout.needDrawBackgroundBorder = YES;
            if (attrs[GGGTextShadowAttributeName] || attrs[NSShadowAttributeName]) layout.needDrawShadow = YES;
            if (attrs[GGGTextUnderlineAttributeName]) layout.needDrawUnderline = YES;
            if (attrs[GGGTextAttachmentAttributeName]) layout.needDrawAttachment = YES;
            if (attrs[GGGTextInnerShadowAttributeName]) layout.needDrawInnerShadow = YES;
            if (attrs[GGGTextStrikethroughAttributeName]) layout.needDrawStrikethrough = YES;
            if (attrs[GGGTextBorderAttributeName]) layout.needDrawBorder = YES;
        };
    ...
    for (NSUInteger i = 0, max = lines.count; i < max; i++) {
        ...
        if (line.attachments.count > 0) {
            ...
            for (GGGTextAttachment *attachment in line.attachments) {
...
+ (NSArray *)layoutWithContainers:(NSArray *)containers text:(NSAttributedString *)text range:(NSRange)range {
    ...
    for (NSUInteger i = 0, max = containers.count; i < max; i++) {
        GGGTextContainer *container = containers[i];
        GGGTextLayout *layout = [self layoutWithContainer:container text:text range:range];
...
- (void)encodeWithCoder:(NSCoder *)aCoder {
    NSData *textData = [GGGTextArchiver archivedDataWithRootObject:_text];
...
- (id)initWithCoder:(NSCoder *)aDecoder {
    ...
    NSAttributedString *text = [GGGTextUnarchiver unarchiveObjectWithData:textData];
    GGGTextContainer *container = [aDecoder decodeObjectForKey:@"container"];
...
- (NSUInteger)_rowIndexForEdge:(CGFloat)edge {
    ...
    while (lo <= hi) {
        mid = (lo + hi) / 2;
        GGGRowEdge oneEdge = _lineRowsEdge[mid];
        if (isVertical ?
 ...
 */
- (CTRunRef)_runForLine:(GGGTextLine *)line position:(GGGTextPosition *)position {
    if (!line || !position) return NULL;
    CFArrayRef runs = CTLineGetGlyphRuns(line.CTLine);
    for (NSUInteger i = 0, max = CFArrayGetCount(runs); i < max; i++) {
        CTRunRef run = CFArrayGetValueAtIndex(runs, i);
        CFRange range = CTRunGetStringRange(run);
        if (position.affinity == GGGTextAffinityBackward) {
...
/**
 ...
 @param block    The block to be executed before returns YES.
            ...
            next:  right position
 */
- (BOOL)_insideComposedCharacterSequences:(GGGTextLine *)line position:(NSUInteger)position block:(void (^)(CGFloat left, CGFloat right, NSUInteger prev, NSUInteger next))block {
 ...
 */
- (BOOL)_insideEmoji:(GGGTextLine *)line position:(NSUInteger)position block:(void (^)(CGFloat left, CGFloat right, NSUInteger prev, NSUInteger next))block {
    if (!line) return NO;
    CFArrayRef runs = CTLineGetGlyphRuns(line.CTLine);
    for (NSUInteger r = 0, rMax = CFArrayGetCount(runs); r < rMax; r++) {
        ...
        if (!GGGTextCTFontContainsColorBitmapGlyphs(font)) continue;
...
/**
 ...
 @return YES if RTL.
 */
- (BOOL)_isRightToLeftInLine:(GGGTextLine *)line atPoint:(CGPoint)point {
    if (!line) return NO;
    // get write direction
    BOOL RTL = NO;
    CFArrayRef runs = CTLineGetGlyphRuns(line.CTLine);
 ...
 */
- (GGGTextRange *)_correctedRangeWithEdge:(GGGTextRange *)range {
    NSRange visibleRange = self.visibleRange;
    GGGTextPosition *start = range.start;
    GGGTextPosition *end = range.end;
    
    if (start.offset == visibleRange.location && start.affinity == GGGTextAffinityBackward) {
        start = [GGGTextPosition positionWithOffset:start.offset affinity:GGGTextAffinityForward];
    }
    
    if (end.offset == visibleRange.location + visibleRange.length && start.affinity == GGGTextAffinityForward) {
        end = [GGGTextPosition positionWithOffset:end.offset affinity:GGGTextAffinityBackward];
    }
    
    if (start != range.start || end != range.end) {
        range = [GGGTextRange rangeWithStart:start end:end];
    }
    return range;
}
...
- (NSUInteger)rowIndexForLine:(NSUInteger)line {
    ...
    return ((GGGTextLine *)_lines[line]).row;
}
...
- (NSUInteger)lineIndexForPoint:(CGPoint)point {
    ...
    for (NSUInteger i = lineIdx0; i <= lineIdx1; i++) {
        CGRect bounds = ((GGGTextLine *)_lines[i]).bounds;
...
- (CGFloat)offsetForTextPosition:(NSUInteger)position lineIndex:(NSUInteger)lineIndex {
    ...
    GGGTextLine *line = _lines[lineIndex];
...
- (NSUInteger)textPositionForPoint:(CGPoint)point lineIndex:(NSUInteger)lineIndex {
    ...
    for (NSUInteger r = 0, max = CFArrayGetCount(runs); r < max; r++) {
        ...
        if (range.location <= idx && idx < range.location + range.length) {
            ...
            CFDictionaryRef attrs = CTRunGetAttributes(run);
            CTFontRef font = CFDictionaryGetValue(attrs, kCTFontAttributeName);
            if (!GGGTextCTFontContainsColorBitmapGlyphs(font)) break;
...
- (GGGTextPosition *)closestPositionToPoint:(CGPoint)point {
    BOOL isVertical = _container.verticalForm;
    ...
    if (position <= _visibleRange.location) {
        return [GGGTextPosition positionWithOffset:_visibleRange.location affinity:GGGTextAffinityForward];
    } else if (position >= _visibleRange.location + _visibleRange.length) {
        return [GGGTextPosition positionWithOffset:_visibleRange.location + _visibleRange.length affinity:GGGTextAffinityBackward];
    }
    
    GGGTextAffinity finalAffinity = GGGTextAffinityForward;
    BOOL finalAffinityDetected = NO;
    
    // binding range
    NSRange bindingRange;
    GGGTextBinding *binding = [_text attribute:GGGTextBindingAttributeName atIndex:position longestEffectiveRange:&bindingRange inRange:NSMakeRange(0, _text.length)];
    if (binding && bindingRange.length > 0) {
        NSUInteger headLineIdx = [self lineIndexForPosition:[GGGTextPosition positionWithOffset:bindingRange.location]];
        NSUInteger tailLineIdx = [self lineIndexForPosition:[GGGTextPosition positionWithOffset:bindingRange.location + bindingRange.length affinity:GGGTextAffinityBackward]];
            ...
            if (left != CGFLOAT_MAX && right != CGFLOAT_MAX) {
                if (_container.isVerticalForm) {
                    if (fabs(point.y - left) < fabs(point.y - right)) {
                        ...
                        finalAffinity = GGGTextAffinityForward;
                    } else {
                        position = bindingRange.location + bindingRange.length;
                        finalAffinity = GGGTextAffinityBackward;
                    }
                } else {
    ...
    if (line.range.length == 0) {
        ...
        return [GGGTextPosition positionWithOffset:line.range.location affinity:behind ? GGGTextAffinityBackward:GGGTextAffinityForward];
    }
    ...
    if (line.range.length <= 2) {
        ...
        if (GGGTextIsLinebreakString(str)) { // an empty line ("\r", "\n", "\r\n")
            return [GGGTextPosition positionWithOffset:line.range.location];
        }
    }
    ...
    if (position >= line.range.location + line.range.length - 1) {
        if (position > line.range.location) {
            ...
            if (GGGTextIsLinebreakChar(c1)) {
                position--;
                if (position > line.range.location) {
                    unichar c0 = [_text.string characterAtIndex:position - 1];
                    if (GGGTextIsLinebreakChar(c0)) {
                        position--;
                    }
                }
            }
    ...
    if (position == line.range.location) {
        return [GGGTextPosition positionWithOffset:position];
    }
    if (position == line.range.location + line.range.length) {
        return [GGGTextPosition positionWithOffset:position affinity:GGGTextAffinityBackward];
    }
    ...
    if (!finalAffinityDetected) {
        ...
        if (ofs != CGFLOAT_MAX) {
            ...
            if (position >= line.range.location + line.range.length) {
                finalAffinity = RTL ? GGGTextAffinityForward : GGGTextAffinityBackward;
            } else if (position <= line.range.location) {
                finalAffinity = RTL ? GGGTextAffinityBackward : GGGTextAffinityForward;
            } else {
                finalAffinity = (ofs < (isVertical ? point.y : point.x) && !RTL) ? GGGTextAffinityForward : GGGTextAffinityBackward;
            }
        }
    }
    
    return [GGGTextPosition positionWithOffset:position affinity:finalAffinity];
}

- (GGGTextPosition *)positionForPoint:(CGPoint)point
                         oldPosition:(GGGTextPosition *)oldPosition
                       otherPosition:(GGGTextPosition *)otherPosition {
    if (!oldPosition || !otherPosition) {
        return oldPosition;
    }
    GGGTextPosition *newPos = [self closestPositionToPoint:point];
    ...
    GGGTextLine *line = _lines[lineIndex];
    GGGRowEdge vertical = _lineRowsEdge[line.row];
    if (_container.verticalForm) {
    ...
    if (_container.isVerticalForm) {
        if ([oldPosition compare:otherPosition] == NSOrderedAscending) { // search backward
            GGGTextRange *range = [self textRangeByExtendingPosition:otherPosition inDirection:UITextLayoutDirectionUp offset:1];
            if (range) return range.start;
        } else { // search forward
            GGGTextRange *range = [self textRangeByExtendingPosition:otherPosition inDirection:UITextLayoutDirectionDown offset:1];
            if (range) return range.end;
        }
    } else {
        if ([oldPosition compare:otherPosition] == NSOrderedAscending) { // search backward
            GGGTextRange *range = [self textRangeByExtendingPosition:otherPosition inDirection:UITextLayoutDirectionLeft offset:1];
            if (range) return range.start;
        } else { // search forward
            GGGTextRange *range = [self textRangeByExtendingPosition:otherPosition inDirection:UITextLayoutDirectionRight offset:1];
            if (range) return range.end;
        }
    }
...
- (GGGTextRange *)textRangeAtPoint:(CGPoint)point {
    ...
    GGGTextPosition *pos = [self closestPositionToPoint:point];
    if (!pos) return nil;
    ...
    if (_container.verticalForm) {
        GGGTextRange *range = [self textRangeByExtendingPosition:pos inDirection:(rect.origin.y >= point.y && !RTL) ? UITextLayoutDirectionUp:UITextLayoutDirectionDown offset:1];
        return range;
    } else {
        GGGTextRange *range = [self textRangeByExtendingPosition:pos inDirection:(rect.origin.x >= point.x && !RTL) ? UITextLayoutDirectionLeft:UITextLayoutDirectionRight offset:1];
        return range;
    }
}

- (GGGTextRange *)closestTextRangeAtPoint:(CGPoint)point {
    GGGTextPosition *pos = [self closestPositionToPoint:point];
    if (!pos) return nil;
    ...
    GGGTextRange *range = [self textRangeByExtendingPosition:pos inDirection:direction offset:1];
    return range;
}

- (GGGTextRange *)textRangeByExtendingPosition:(GGGTextPosition *)position {
    ...
    if (position.offset == visibleStart) {
        return [GGGTextRange rangeWithRange:NSMakeRange(position.offset, 0)];
    } else if (position.offset == visibleEnd) {
        return [GGGTextRange rangeWithRange:NSMakeRange(position.offset, 0) affinity:GGGTextAffinityBackward];
    }
    
    // binding range
    NSRange tRange;
    GGGTextBinding *binding = [_text attribute:GGGTextBindingAttributeName atIndex:position.offset longestEffectiveRange:&tRange inRange:_visibleRange];
    if (binding && tRange.length > 0 && tRange.location < position.offset) {
        return [GGGTextRange rangeWithRange:tRange];
    }
    ...
    if (lineIndex != NSNotFound) {
        ...
        if (emoji || seq) {
            return [GGGTextRange rangeWithRange:NSMakeRange(_prev, _next - _prev)];
        }
    }
    ...
    if (position.offset > visibleStart && position.offset < visibleEnd) {
        ...
        if ((c0 == '\r') && position.offset < visibleEnd) {
            ...
            if (c1 == '\n') {
                return [GGGTextRange rangeWithStart:[GGGTextPosition positionWithOffset:position.offset - 1] end:[GGGTextPosition positionWithOffset:position.offset + 1]];
            }
        }
        if (GGGTextIsLinebreakChar(c0) && position.affinity == GGGTextAffinityBackward) {
            NSString *str = [_text.string substringToIndex:position.offset];
            NSUInteger len = GGGTextLinebreakTailLength(str);
            return [GGGTextRange rangeWithStart:[GGGTextPosition positionWithOffset:position.offset - len] end:[GGGTextPosition positionWithOffset:position.offset]];
        }
    }
    
    return [GGGTextRange rangeWithRange:NSMakeRange(position.offset, 0) affinity:position.affinity];
}
...
- (GGGTextRange *)textRangeByExtendingPosition:(GGGTextPosition *)position
    ...
    if (!forwardMove && position.offset == visibleStart) {
        return [GGGTextRange rangeWithRange:NSMakeRange(_visibleRange.location, 0)];
    } else if (forwardMove && position.offset == visibleEnd) {
        return [GGGTextRange rangeWithRange:NSMakeRange(position.offset, 0) affinity:GGGTextAffinityBackward];
    }
    
    // extend from position
    GGGTextRange *fromRange = [self textRangeByExtendingPosition:position];
    if (!fromRange) return nil;
    GGGTextRange *allForward = [GGGTextRange rangeWithStart:fromRange.start end:[GGGTextPosition positionWithOffset:visibleEnd]];
    GGGTextRange *allBackward = [GGGTextRange rangeWithStart:[GGGTextPosition positionWithOffset:visibleStart] end:fromRange.end];
    ...
    if (verticalMove) { // up/down in text layout
        ...
        GGGTextLine *mostLeftLine = nil, *mostRightLine = nil;
        ...
        GGGTextLine *insideLine = _lines[insideIndex];
        NSUInteger pos;
        if (isVerticalForm) {
        ...
        GGGTextPosition *extPos;
        if (afinityEdge) {
            if (pos == insideLine.range.location + insideLine.range.length) {
                NSString *subStr = [_text.string substringWithRange:insideLine.range];
                NSUInteger lineBreakLen = GGGTextLinebreakTailLength(subStr);
                extPos = [GGGTextPosition positionWithOffset:pos - lineBreakLen];
            } else {
                extPos = [GGGTextPosition positionWithOffset:pos];
            }
        } else {
            extPos = [GGGTextPosition positionWithOffset:pos];
        }
        GGGTextRange *ext = [self textRangeByExtendingPosition:extPos];
        if (!ext) return nil;
        if (forwardMove) {
            return [GGGTextRange rangeWithStart:fromRange.start end:ext.end];
        } else {
            return [GGGTextRange rangeWithStart:ext.start end:fromRange.end];
        }
        
    } else { // left/right in text layout
        GGGTextPosition *toPosition = [GGGTextPosition positionWithOffset:position.offset + (forwardMove ? offset : -offset)];
        if (toPosition.offset <= visibleStart) return allBackward;
        else if (toPosition.offset >= visibleEnd) return allForward;
        
        GGGTextRange *toRange = [self textRangeByExtendingPosition:toPosition];
        ...
        return [GGGTextRange rangeWithRange:NSMakeRange(start, end - start)];
    }
}

- (NSUInteger)lineIndexForPosition:(GGGTextPosition *)position {
...
- (CGPoint)linePositionForPosition:(GGGTextPosition *)position {
    NSUInteger lineIndex = [self lineIndexForPosition:position];
...
- (CGRect)caretRectForPosition:(GGGTextPosition *)position {
    NSUInteger lineIndex = [self lineIndexForPosition:position];
...
- (CGRect)firstRectForRange:(GGGTextRange *)range {
    ...
    GGGTextLine *startLine = _lines[startLineIndex];
    GGGTextLine *endLine = _lines[endLineIndex];
    NSMutableArray *lines = [NSMutableArray new];
    for (NSUInteger i = startLineIndex; i <= startLineIndex; i++) {
        GGGTextLine *line = _lines[i];
    ...
    if (_container.verticalForm) {
        if (lines.count == 1) {
            ...
            if (top > bottom) GGGTEXT_SWAP(top, bottom);
        ...
        if (lines.count == 1) {
            ...
            if (left > right) GGGTEXT_SWAP(left, right);
...
- (CGRect)rectForRange:(GGGTextRange *)range {
    NSArray *rects = [self selectionRectsForRange:range];
    if (rects.count == 0) return CGRectNull;
    CGRect rectUnion = ((GGGTextSelectionRect *)rects.firstObject).rect;
    for (NSUInteger i = 1; i < rects.count; i++) {
        GGGTextSelectionRect *rect = rects[i];
        rectUnion = CGRectUnion(rectUnion, rect.rect);
    }
    return rectUnion;
}

- (NSArray *)selectionRectsForRange:(GGGTextRange *)range {
    range = [self _correctedRangeWithEdge:range];
    ...
    if (startLineIndex > endLineIndex) GGGTEXT_SWAP(startLineIndex, endLineIndex);
    GGGTextLine *startLine = _lines[startLineIndex];
    GGGTextLine *endLine = _lines[endLineIndex];
    ...
    GGGTextSelectionRect *start = [GGGTextSelectionRect new];
    if (isVertical) {
    ...
    GGGTextSelectionRect *end = [GGGTextSelectionRect new];
    if (isVertical) {
    ...
    if (startLine.row == endLine.row) { // same row
        if (offsetStart > offsetEnd) GGGTEXT_SWAP(offsetStart, offsetEnd);
        GGGTextSelectionRect *rect = [GGGTextSelectionRect new];
        if (isVertical) {
        ...
        GGGTextSelectionRect *topRect = [GGGTextSelectionRect new];
        ...
        GGGTextSelectionRect *bottomRect = [GGGTextSelectionRect new];
...
- (NSArray *)selectionRectsWithoutStartAndEndForRange:(GGGTextRange *)range {
...
- (NSArray *)selectionRectsWithOnlyStartAndEndForRange:(GGGTextRange *)range {
    NSMutableArray *rects = [self selectionRectsForRange:range].mutableCopy;
    for (NSInteger i = 0, max = rects.count; i < max; i++) {
        GGGTextSelectionRect *rect = rects[i];
        if (!rect.containsStart && !rect.containsEnd) {
...
typedef NS_OPTIONS(NSUInteger, GGGTextDecorationType) {
    GGGTextDecorationTypeUnderline     = 1 << 0,
    GGGTextDecorationTypeStrikethrough = 1 << 1,
};

typedef NS_OPTIONS(NSUInteger, GGGTextBorderType) {
    GGGTextBorderTypeBackgound = 1 << 0,
    GGGTextBorderTypeNormal    = 1 << 1,
};

static CGRect GGGTextMergeRectInSameLine(CGRect rect1, CGRect rect2, BOOL isVertical) {
    if (isVertical) {
...
static void GGGTextGetRunsMaxMetric(CFArrayRef runs, CGFloat *xHeight, CGFloat *underlinePosition, CGFloat *lineThickness) {
...
static void GGGTextDrawRun(GGGTextLine *line, CTRunRef run, CGContextRef context, CGSize size, BOOL isVertical, NSArray *runRanges, CGFloat verticalOffset) {
    CGAffineTransform runTextMatrix = CTRunGetTextMatrix(run);
    BOOL runTextMatrixIsID = CGAffineTransformIsIdentity(runTextMatrix);
    
    CFDictionaryRef runAttrs = CTRunGetAttributes(run);
    NSValue *glyphTransformValue = CFDictionaryGetValue(runAttrs, (__bridge const void *)(GGGTextGlyphTransformAttributeName));
    if (!isVertical && !glyphTransformValue) { // draw run
        ...
        fillColor = GGGTextGetCGColor(fillColor);
        ...
        CGContextSaveGState(context); {
            ...
            if (isVertical) {
                ...
                for (GGGTextRunGlyphRange *oneRange in runRanges) {
                    NSRange range = oneRange.glyphRangeInRun;
                    NSUInteger rangeMax = range.location + range.length;
                    GGGTextRunGlyphDrawMode mode = oneRange.drawMode;
                    ...
                    for (NSUInteger g = range.location; g < rangeMax; g++) {
                        CGContextSaveGState(context); {
                            ...
                            if (mode) { // CJK glyph, need rotated
                                ...
                                if (mode == GGGTextRunGlyphDrawModeVerticalRotateMove) {
                                    x += w;
                                    y += w;
                                }
                                CGContextSetTextPosition(context, x, y);
                            } else {
                                CGContextRotateCTM(context, GGGTextDegreesToRadians(-90));
                                CGContextSetTextPosition(context,
                                                         line.position.y - size.height + glyphPositions[g].x,
                                                         line.position.x + verticalOffset + glyphPositions[g].y);
                            }
                            
                            if (GGGTextCTFontContainsColorBitmapGlyphs(runFont)) {
...
static void GGGTextSetLinePatternInContext(GGGTextLineStyle style, CGFloat width, CGFloat phase, CGContextRef context){
    ...
    } else if (pattern == GGGTextLineStylePatternCircleDot) {
...
static void GGGTextDrawBorderRects(CGContextRef context, CGSize size, GGGTextBorder *border, NSArray *rects, BOOL isVertical) {
    if (rects.count == 0) return;
    
    GGGTextShadow *shadow = border.shadow;
    if (shadow.color) {
        CGContextSaveGState(context);
    ...
    for (NSValue *value in rects) {
        ...
        rect = GGGTextCGRectPixelRound(rect);
    ...
    if (border.strokeColor && border.lineStyle > 0 && border.strokeWidth > 0) {
        ...
        GGGTextSetLinePatternInContext(border.lineStyle, border.strokeWidth, 0, context);
        CGFloat inset = -border.strokeWidth * 0.5;
        if ((border.lineStyle & 0xFF) == GGGTextLineStyleThick) {
            inset *= 2;
        ...
        //------------------------- second line ------------------------------//
        if ((border.lineStyle & 0xFF) == GGGTextLineStyleDouble) {
            CGContextSaveGState(context);
...
static void GGGTextDrawLineStyle(CGContextRef context, CGFloat length, CGFloat lineWidth, GGGTextLineStyle style, CGPoint position, CGColorRef color, BOOL isVertical) {
    ...
    CGContextSaveGState(context); {
        if (isVertical) {
            CGFloat x, y1, y2, w;
            y1 = GGGTextCGFloatPixelRound(position.y);
            y2 = GGGTextCGFloatPixelRound(position.y + length);
            w = (styleBase == GGGTextLineStyleThick ? lineWidth * 2 : lineWidth);
            
            CGFloat linePixel = GGGTextCGFloatToPixel(w);
            if (fabs(linePixel - floor(linePixel)) < 0.1) {
                int iPixel = linePixel;
                if (iPixel == 0 || (iPixel % 2)) { // odd line pixel
                    x = GGGTextCGFloatPixelHalf(position.x);
                } else {
                    x = GGGTextCGFloatPixelFloor(position.x);
                }
            } else {
                x = position.x;
            }
            ...
            GGGTextSetLinePatternInContext(style, lineWidth, position.y, context);
            CGContextSetLineWidth(context, w);
            if (styleBase == GGGTextLineStyleSingle) {
                CGContextMoveToPoint(context, x, y1);
                CGContextAddLineToPoint(context, x, y2);
                CGContextStrokePath(context);
            } else if (styleBase == GGGTextLineStyleThick) {
                CGContextMoveToPoint(context, x, y1);
                CGContextAddLineToPoint(context, x, y2);
                CGContextStrokePath(context);
            } else if (styleBase == GGGTextLineStyleDouble) {
            ...
            CGFloat x1, x2, y, w;
            x1 = GGGTextCGFloatPixelRound(position.x);
            x2 = GGGTextCGFloatPixelRound(position.x + length);
            w = (styleBase == GGGTextLineStyleThick ? lineWidth * 2 : lineWidth);
            ...
            if (fabs(linePixel - floor(linePixel)) < 0.1) {
                int iPixel = linePixel;
                if (iPixel == 0 || (iPixel % 2)) { // odd line pixel
                    y = GGGTextCGFloatPixelHalf(position.y);
                } else {
                    y = GGGTextCGFloatPixelFloor(position.y);
                }
            } else {
                y = position.y;
            }
            ...
            CGContextSetStrokeColorWithColor(context, color);
            GGGTextSetLinePatternInContext(style, lineWidth, position.x, context);
            CGContextSetLineWidth(context, w);
            if (styleBase == GGGTextLineStyleSingle) {
...
static void GGGTextDrawText(GGGTextLayout *layout, CGContextRef context, CGSize size, CGPoint point, BOOL (^cancel)(void)) {
    CGContextSaveGState(context); {
        ...
        for (NSUInteger l = 0, lMax = lines.count; l < lMax; l++) {
            ...
            for (NSUInteger r = 0, rMax = CFArrayGetCount(runs); r < rMax; r++) {
                ...
                GGGTextDrawRun(line, run, context, size, isVertical, lineRunRanges[r], verticalOffset);
            }
...
static void GGGTextDrawBlockBorder(GGGTextLayout *layout, CGContextRef context, CGSize size, CGPoint point, BOOL (^cancel)(void)) {
    CGContextSaveGState(context);
    CGContextTranslateCTM(context, point.x, point.y);
    ...
    for (NSInteger l = 0, lMax = lines.count; l < lMax; l++) {
        ...
        for (NSInteger r = 0, rMax = CFArrayGetCount(runs); r < rMax; r++) {
            ...
            GGGTextBorder *border = attrs[GGGTextBlockBorderAttributeName];
            if (!border) continue;
            
            NSUInteger lineStartIndex = line.index;
            while (lineStartIndex > 0) {
                if (((GGGTextLine *)lines[lineStartIndex - 1]).row == line.row) lineStartIndex--;
                else break;
            }
            
            CGRect unionRect = CGRectZero;
            NSUInteger lineStartRow = ((GGGTextLine *)lines[lineStartIndex]).row;
            NSUInteger lineContinueIndex = lineStartIndex;
            NSUInteger lineContinueRow = lineStartRow;
            do {
                GGGTextLine *one = lines[lineContinueIndex];
                ...
                GGGTextLine *next = lines[lineContinueIndex + 1];
                if (next.row != lineContinueRow) {
                    GGGTextBorder *nextBorder = [layout.text yy_attribute:GGGTextBlockBorderAttributeName atIndex:next.range.location];
            ...
            GGGTextDrawBorderRects(context, size, border, @[[NSValue valueWithCGRect:unionRect]], isVertical);
...
static void GGGTextDrawBorder(GGGTextLayout *layout, CGContextRef context, CGSize size, CGPoint point, GGGTextBorderType type, BOOL (^cancel)(void)) {
    CGContextSaveGState(context);
    CGContextTranslateCTM(context, point.x, point.y);
    ...
    NSArray *lines = layout.lines;
    NSString *borderKey = (type == GGGTextBorderTypeNormal ? GGGTextBorderAttributeName : GGGTextBackgroundBorderAttributeName);
    ...
    for (NSInteger l = 0, lMax = lines.count; l < lMax; l++) {
        ...
        for (NSInteger r = 0, rMax = CFArrayGetCount(runs); r < rMax; r++) {
            ...
            NSDictionary *attrs = (id)CTRunGetAttributes(run);
            GGGTextBorder *border = attrs[borderKey];
            if (!border) continue;
            ...
            for (NSInteger ll = l; ll < lMax; ll++) {
                ...
                for (NSInteger rr = (ll == l) ? r : 0, rrMax = CFArrayGetCount(iRuns); rr < rrMax; rr++) {
                    ...
                    GGGTextBorder *iBorder = iAttrs[borderKey];
                    ...
                    if (isVertical) {
                        GGGTEXT_SWAP(iRunPosition.x, iRunPosition.y);
            ...
            for (NSInteger re = 0, reMax = runRects.count; re < reMax; re++) {
                ...
                if (isVertical) {
                    if (fabs(rect.origin.x - curRect.origin.x) < 1) {
                        curRect = GGGTextMergeRectInSameLine(rect, curRect, isVertical);
                    } else {
            ...
            GGGTextDrawBorderRects(context, size, border, drawRects, isVertical);
...
static void GGGTextDrawDecoration(GGGTextLayout *layout, CGContextRef context, CGSize size, CGPoint point, GGGTextDecorationType type, BOOL (^cancel)(void)) {
    NSArray *lines = layout.lines;
    ...
    for (NSUInteger l = 0, lMax = layout.lines.count; l < lMax; l++) {
        ...
        for (NSUInteger r = 0, rMax = CFArrayGetCount(runs); r < rMax; r++) {
            ...
            NSDictionary *attrs = (id)CTRunGetAttributes(run);
            GGGTextDecoration *underline = attrs[GGGTextUnderlineAttributeName];
            GGGTextDecoration *strikethrough = attrs[GGGTextStrikethroughAttributeName];
            
            BOOL needDrawUnderline = NO, needDrawStrikethrough = NO;
            if ((type & GGGTextDecorationTypeUnderline) && underline.style > 0) {
                needDrawUnderline = YES;
            }
            if ((type & GGGTextDecorationTypeStrikethrough) && strikethrough.style > 0) {
            ...
            if (GGGTextIsLinebreakString(runStr)) continue; // may need more checks...
            
            CGFloat xHeight, underlinePosition, lineThickness;
            GGGTextGetRunsMaxMetric(runs, &xHeight, &underlinePosition, &lineThickness);
                ...
                while (shadow) {
                    ...
                    CGContextSaveGState(context); {
                        ...
                        CGContextSaveGState(context); {
                            ...
                            GGGTextDrawLineStyle(context, length, thickness, underline.style, underlineStart, color, isVertical);
                        } CGContextRestoreGState(context);
                    } CGContextRestoreGState(context);
                    shadow = shadow.subShadow;
                }
            ...
            if (needDrawStrikethrough) {
                ...
                GGGTextDrawLineStyle(context, length, thickness, strikethrough.style, strikethroughStart, color, isVertical);
            }
        }
    }
    CGContextRestoreGState(context);
...
static void GGGTextDrawAttachment(GGGTextLayout *layout, CGContextRef context, CGSize size, CGPoint point, UIView *targetView, CALayer *targetLayer, BOOL (^cancel)(void)) {
    
    BOOL isVertical = layout.container.verticalForm;
    CGFloat verticalOffset = isVertical ? (size.width - layout.container.size.width) : 0;
    
    for (NSUInteger i = 0, max = layout.attachments.count; i < max; i++) {
        GGGTextAttachment *a = layout.attachments[i];
        ...
        rect = GGGTextCGRectFitWithContentMode(rect, asize, a.contentMode);
        rect = GGGTextCGRectPixelRound(rect);
        rect = CGRectStandardize(rect);
...
static void GGGTextDrawShadow(GGGTextLayout *layout, CGContextRef context, CGSize size, CGPoint point, BOOL (^cancel)(void)) {
    ...
    CGContextSaveGState(context); {
        ...
        for (NSUInteger l = 0, lMax = layout.lines.count; l < lMax; l++) {
            ...
            for (NSUInteger r = 0, rMax = CFArrayGetCount(runs); r < rMax; r++) {
                ...
                NSDictionary *attrs = (id)CTRunGetAttributes(run);
                GGGTextShadow *shadow = attrs[GGGTextShadowAttributeName];
                GGGTextShadow *nsShadow = [GGGTextShadow shadowWithNSShadow:attrs[NSShadowAttributeName]]; // NSShadow compatible
                if (nsShadow) {
...
static void GGGTextDrawInnerShadow(GGGTextLayout *layout, CGContextRef context, CGSize size, CGPoint point, BOOL (^cancel)(void)) {
    CGContextSaveGState(context);
    CGContextTranslateCTM(context, point.x, point.y);
    CGContextTranslateCTM(context, 0, size.height);
    CGContextScaleCTM(context, 1, -1);
    ...
    for (NSUInteger l = 0, lMax = lines.count; l < lMax; l++) {
        ...
        for (NSUInteger r = 0, rMax = CFArrayGetCount(runs); r < rMax; r++) {
            ...
            CGContextSetTextPosition(context, linePosX, linePosY);
            NSDictionary *attrs = (id)CTRunGetAttributes(run);
            GGGTextShadow *shadow = attrs[GGGTextInnerShadowAttributeName];
            while (shadow) {
                if (!shadow.color) {
                    shadow = shadow.subShadow;
                    continue;
...
static void GGGTextDrawDebug(GGGTextLayout *layout, CGContextRef context, CGSize size, CGPoint point, GGGTextDebugOption *op) {
    UIGraphicsPushContext(context);
    CGContextSaveGState(context);
    CGContextTranslateCTM(context, point.x, point.y);
    CGContextSetLineWidth(context, 1.0 / GGGTextScreenScale());
    ...
    if (op.CTFrameBorderColor || op.CTFrameFillColor) {
        ...
        if (!path) {
            ...
            if (op.CTFrameBorderColor) rect = GGGTextCGRectPixelHalf(rect);
            else rect = GGGTextCGRectPixelRound(rect);
    ...
    for (NSUInteger l = 0, lMax = lines.count; l < lMax; l++) {
        ...
        if (op.CTLineFillColor) {
            ...
            CGContextAddRect(context, GGGTextCGRectPixelRound(lineBounds));
            CGContextFillPath(context);
        }
        if (op.CTLineBorderColor) {
            [op.CTLineBorderColor setStroke];
            CGContextAddRect(context, GGGTextCGRectPixelHalf(lineBounds));
            CGContextStrokePath(context);
        }
        if (op.baselineColor) {
            [op.baselineColor setStroke];
            if (isVertical) {
                CGFloat x = GGGTextCGFloatPixelHalf(line.position.x);
                CGFloat y1 = GGGTextCGFloatPixelHalf(line.top);
                CGFloat y2 = GGGTextCGFloatPixelHalf(line.bottom);
                CGContextMoveToPoint(context, x, y1);
                CGContextAddLineToPoint(context, x, y2);
                CGContextStrokePath(context);
            } else {
                CGFloat x1 = GGGTextCGFloatPixelHalf(lineBounds.origin.x);
                CGFloat x2 = GGGTextCGFloatPixelHalf(lineBounds.origin.x + lineBounds.size.width);
                CGFloat y = GGGTextCGFloatPixelHalf(line.position.y);
                CGContextMoveToPoint(context, x1, y);
                CGContextAddLineToPoint(context, x2, y);
                CGContextStrokePath(context);
            }
        ...
        if (op.CTRunFillColor || op.CTRunBorderColor || op.CTRunNumberColor || op.CGGlyphFillColor || op.CGGlyphBorderColor) {
            ...
            for (NSUInteger r = 0, rMax = CFArrayGetCount(runs); r < rMax; r++) {
                ...
                if (isVertical) {
                    GGGTEXT_SWAP(runPosition.x, runPosition.y);
                ...
                if (op.CTRunFillColor) {
                    ...
                    CGContextAddRect(context, GGGTextCGRectPixelRound(runTypoBounds));
                    CGContextFillPath(context);
                }
                if (op.CTRunBorderColor) {
                    [op.CTRunBorderColor setStroke];
                    CGContextAddRect(context, GGGTextCGRectPixelHalf(runTypoBounds));
                    CGContextStrokePath(context);
                }
                ...
                if (op.CGGlyphBorderColor || op.CGGlyphFillColor) {
                    for (NSUInteger g = 0; g < glyphCount; g++) {
                        ...
                        if (op.CGGlyphFillColor) {
                            ...
                            CGContextAddRect(context, GGGTextCGRectPixelRound(rect));
                            CGContextFillPath(context);
                        }
                        if (op.CGGlyphBorderColor) {
                            [op.CGGlyphBorderColor setStroke];
                            CGContextAddRect(context, GGGTextCGRectPixelHalf(rect));
                            CGContextStrokePath(context);
                        }
                    }
                }
...
- (void)drawInContext:(CGContextRef)context
                ...
                point:(CGPoint)point
                 view:(UIView *)view
                layer:(CALayer *)layer
                debug:(GGGTextDebugOption *)debug
                cancel:(BOOL (^)(void))cancel{
    @autoreleasepool {
        if (self.needDrawBlockBorder && context) {
            if (cancel && cancel()) return;
            GGGTextDrawBlockBorder(self, context, size, point, cancel);
        }
        if (self.needDrawBackgroundBorder && context) {
            if (cancel && cancel()) return;
            GGGTextDrawBorder(self, context, size, point, GGGTextBorderTypeBackgound, cancel);
        }
        if (self.needDrawShadow && context) {
            if (cancel && cancel()) return;
            GGGTextDrawShadow(self, context, size, point, cancel);
        }
        if (self.needDrawUnderline && context) {
            if (cancel && cancel()) return;
            GGGTextDrawDecoration(self, context, size, point, GGGTextDecorationTypeUnderline, cancel);
        }
        if (self.needDrawText && context) {
            if (cancel && cancel()) return;
            GGGTextDrawText(self, context, size, point, cancel);
        }
        if (self.needDrawAttachment && (context || view || layer)) {
            if (cancel && cancel()) return;
            GGGTextDrawAttachment(self, context, size, point, view, layer, cancel);
        }
        if (self.needDrawInnerShadow && context) {
            if (cancel && cancel()) return;
            GGGTextDrawInnerShadow(self, context, size, point, cancel);
        }
        if (self.needDrawStrikethrough && context) {
            if (cancel && cancel()) return;
            GGGTextDrawDecoration(self, context, size, point, GGGTextDecorationTypeStrikethrough, cancel);
        }
        if (self.needDrawBorder && context) {
            if (cancel && cancel()) return;
            GGGTextDrawBorder(self, context, size, point, GGGTextBorderTypeNormal, cancel);
        }
        if (debug.needDrawDebug && context) {
            if (cancel && cancel()) return;
            GGGTextDrawDebug(self, context, size, point, debug);
        }
    }
}
...
- (void)removeAttachmentFromViewAndLayer {
    NSAssert([NSThread isMainThread], @"This method must be called on the main thread");
    for (GGGTextAttachment *a in self.attachments) {
