...
static int _GGGTextKeyboardViewFrameObserverKey;

/// Observer for view's frame/bounds/center/transform
@interface _GGGTextKeyboardViewFrameObserver : NSObject
...
@implementation _GGGTextKeyboardViewFrameObserver {
...
- (void)addToKeyboardView:(UIView *)keyboardView {
    ...
    if (_keyboardView) {
        ...
        objc_setAssociatedObject(_keyboardView, &_GGGTextKeyboardViewFrameObserverKey, nil, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    }
    _keyboardView = keyboardView;
    if (keyboardView) {
        [self addFrameObserver];
    }
    objc_setAssociatedObject(keyboardView, &_GGGTextKeyboardViewFrameObserverKey, self, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
...
+ (instancetype)observerForView:(UIView *)keyboardView {
    ...
    return objc_getAssociatedObject(keyboardView, &_GGGTextKeyboardViewFrameObserverKey);
}
...
@implementation GGGTextKeyboardManager {
...
- (instancetype)init {
    @throw [NSException exceptionWithName:@"GGGTextKeyboardManager init error" reason:@"Use 'defaultManager' to get instance." userInfo:nil];
    return [super init];
}
...
- (void)_initFrameObserver {
    ...
    _GGGTextKeyboardViewFrameObserver *observer = [_GGGTextKeyboardViewFrameObserver observerForView:keyboardView];
    if (!observer) {
        observer = [_GGGTextKeyboardViewFrameObserver new];
...
+ (instancetype)defaultManager {
    static GGGTextKeyboardManager *mgr = nil;
...
- (void)addObserver:(id<GGGTextKeyboardObserver>)observer {
    if (!observer) return;
    [_observers addObject:observer];
}

- (void)removeObserver:(id<GGGTextKeyboardObserver>)observer {
    if (!observer) return;
...
- (void)_notifyAllObservers {
    ...
    GGGTextKeyboardTransition trans = {0};
    ...
    if (!CGRectEqualToRect(trans.toFrame, _fromFrame)) {
        for (id<GGGTextKeyboardObserver> observer in _observers.copy) {
