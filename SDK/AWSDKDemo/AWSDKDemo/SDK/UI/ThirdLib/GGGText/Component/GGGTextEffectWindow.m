...
@implementation GGGTextEffectWindow

+ (instancetype)sharedWindow {
    static GGGTextEffectWindow *one = nil;
    if (one == nil) {
        // iOS 9 compatible
    ...
    dispatch_once(&onceToken, ^{
        if (!GGGTextIsAppExtension()) {
            one = [self new];
            one.frame = (CGRect){.size = GGGTextScreenSize()};
...
- (void)becomeKeyWindow {
    [[GGGTextSharedApplication().delegate window] makeKeyWindow];
}

- (UIViewController *)rootViewController {
    for (UIWindow *window in [GGGTextSharedApplication() windows]) {
...
- (GGGTextDirection)_keyboardDirection {
    CGRect keyboardFrame = [GGGTextKeyboardManager defaultManager].keyboardFrame;
    keyboardFrame = [[GGGTextKeyboardManager defaultManager] convertRect:keyboardFrame toView:self];
    if (CGRectIsNull(keyboardFrame) || CGRectIsEmpty(keyboardFrame)) return GGGTextDirectionNone;
    
    if (CGRectGetMinY(keyboardFrame) == 0 &&
        CGRectGetMinX(keyboardFrame) == 0 &&
        CGRectGetMaxX(keyboardFrame) == CGRectGetWidth(self.frame))
        return GGGTextDirectionTop;
    ...
    if (CGRectGetMaxX(keyboardFrame) == CGRectGetWidth(self.frame) &&
        ...
        return GGGTextDirectionRight;
    
    if (CGRectGetMaxY(keyboardFrame) == CGRectGetHeight(self.frame) &&
        CGRectGetMinX(keyboardFrame) == 0 &&
        CGRectGetMaxX(keyboardFrame) == CGRectGetWidth(self.frame))
        return GGGTextDirectionBottom;
    
    if (CGRectGetMinX(keyboardFrame) == 0 &&
        CGRectGetMinY(keyboardFrame) == 0 &&
        CGRectGetMaxY(keyboardFrame) == CGRectGetHeight(self.frame))
        return GGGTextDirectionLeft;
    
    return GGGTextDirectionNone;
}
...
- (CGPoint)_correctedCenter:(CGPoint)center forMagnifier:(GGGTextMagnifier *)mag rotation:(CGFloat)rotation {
    CGFloat degree = GGGTextRadiansToDegrees(rotation);
    ...
    if (degree <= 1 || degree >= 7) { //top
        if (mag.type == GGGTextMagnifierTypeCaret) {
            if (center.y < caretExt)
                center.y = caretExt;
        } else if (mag.type == GGGTextMagnifierTypeRanged) {
 ...
 */
- (CGFloat)_updateMagnifier:(GGGTextMagnifier *)mag {
    UIApplication *app = GGGTextSharedApplication();
    if (!app) return 0;
    ...
    CGAffineTransform trans = GGGTextCGAffineTransformGetFromViews(hostView, self);
    CGFloat rotation = GGGTextCGAffineTransformGetRotation(trans);
    ...
    for (UIWindow *window in windows) {
        ...
        CGContextConcatCTM(context, GGGTextCGAffineTransformGetFromViews(window, self));
...
- (void)showMagnifier:(GGGTextMagnifier *)mag {
    if (!mag) return;
    ...
    mag.transform = trans;
    mag.center = center;
    if (mag.type == GGGTextMagnifierTypeRanged) {
        mag.alpha = 0;
    }
    NSTimeInterval time = mag.type == GGGTextMagnifierTypeCaret ? 0.08 : 0.1;
...
- (void)moveMagnifier:(GGGTextMagnifier *)mag {
    if (!mag) return;
    [self _updateWindowLevel];
    CGFloat rotation = [self _updateMagnifier:mag];
    CGPoint center = [self yy_convertPoint:mag.hostPopoverCenter fromViewOrWindow:mag.hostView];
...
- (void)hideMagnifier:(GGGTextMagnifier *)mag {
    if (!mag) return;
    if (mag.superview != self) return;
    CGFloat rotation = [self _updateMagnifier:mag];
    CGPoint center = [self yy_convertPoint:mag.hostPopoverCenter fromViewOrWindow:mag.hostView];
    NSTimeInterval time = mag.type == GGGTextMagnifierTypeCaret ? 0.20 : 0.15;
    [UIView animateWithDuration:time delay:0 options:UIViewAnimationOptionCurveEaseInOut | UIViewAnimationOptionAllowUserInteraction | UIViewAnimationOptionBeginFromCurrentState animations:^{
...
- (void)_updateSelectionGrabberDot:(GGGSelectionGrabberDot *)dot selection:(GGGTextSelectionView *)selection{
    ...
    if (selection.hostView.clipsToBounds == YES && dot.yy_visibleAlpha > 0.1) {
        ...
        if (!dotInKeyboard) {
            ...
            if (GGGTextCGRectGetArea(intersection) < GGGTextCGRectGetArea(dotRect)) {
                CGFloat dist = GGGTextCGPointGetDistanceToRect(GGGTextCGRectGetCenter(dotRect), hostRect);
...
- (void)showSelectionDot:(GGGTextSelectionView *)selection {
    if (!selection) return;
    [self _updateWindowLevel];
...
- (void)hideSelectionDot:(GGGTextSelectionView *)selection {
    if (!selection) return;
