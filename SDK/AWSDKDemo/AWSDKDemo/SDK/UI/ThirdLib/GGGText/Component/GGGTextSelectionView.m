...
@implementation GGGSelectionGrabberDot
...
@implementation GGGSelectionGrabber

- (instancetype) initWithFrame:(CGRect)frame {
    self = [super initWithFrame:frame];
    if (!self) return nil;
    _dot = [[GGGSelectionGrabberDot alloc] initWithFrame:CGRectMake(0, 0, 10, 10)];
    return self;
}

- (void)setDotDirection:(GGGTextDirection)dotDirection {
    _dotDirection = dotDirection;
    [self addSubview:_dot];
    CGRect frame = _dot.frame;
    CGFloat ofs = 0.5;
    if (dotDirection == GGGTextDirectionTop) {
        frame.origin.y = -frame.size.height + ofs;
        frame.origin.x = (self.bounds.size.width - frame.size.width) / 2;
    } else if (dotDirection == GGGTextDirectionRight) {
        frame.origin.x = self.bounds.size.width - ofs;
        frame.origin.y = (self.bounds.size.height - frame.size.height) / 2;
    } else if (dotDirection == GGGTextDirectionBottom) {
        frame.origin.y = self.bounds.size.height - ofs;
        frame.origin.x = (self.bounds.size.width - frame.size.width) / 2;
    } else if (dotDirection == GGGTextDirectionLeft) {
...
- (CGRect)touchRect {
    ...
    if (_dotDirection == GGGTextDirectionTop) {
        insets.top = -kTouchDotExtend;
    } else if (_dotDirection == GGGTextDirectionRight) {
        insets.right = -kTouchDotExtend;
    } else if (_dotDirection == GGGTextDirectionBottom) {
        insets.bottom = -kTouchDotExtend;
    } else if (_dotDirection == GGGTextDirectionLeft) {
...
@interface GGGTextSelectionView ()
@property (nonatomic, strong) NSTimer *caretTimer;
@property (nonatomic, strong) UIView *caretView;
@property (nonatomic, strong) GGGSelectionGrabber *startGrabber;
@property (nonatomic, strong) GGGSelectionGrabber *endGrabber;
@property (nonatomic, strong) NSMutableArray *markViews;
@end

@implementation GGGTextSelectionView
...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    _startGrabber = [GGGSelectionGrabber new];
    _startGrabber.dotDirection = GGGTextDirectionTop;
    _startGrabber.hidden = YES;
    _endGrabber = [GGGSelectionGrabber new];
    _endGrabber.dotDirection = GGGTextDirectionBottom;
...
- (void)_startBlinks {
    [_caretTimer invalidate];
    if (_caretVisible) {
        _caretTimer = [NSTimer timerWithTimeInterval:kBlinkDuration target:[GGGTextWeakProxy proxyWithTarget:self] selector:@selector(_doBlink) userInfo:nil repeats:YES];
...
- (void)setVerticalForm:(BOOL)verticalForm {
    if (_verticalForm != verticalForm) {
        ...
        self.startGrabber.dotDirection = verticalForm ? GGGTextDirectionRight : GGGTextDirectionTop;
        self.endGrabber.dotDirection = verticalForm ? GGGTextDirectionLeft : GGGTextDirectionBottom;
    }
}
...
- (CGRect)_standardCaretRect:(CGRect)caretRect {
    ...
    caretRect = GGGTextCGRectPixelRound(caretRect);
...
- (void)setSelectionRects:(NSArray *)selectionRects {
    ...
    [selectionRects enumerateObjectsUsingBlock: ^(GGGTextSelectionRect *r, NSUInteger idx, BOOL *stop) {
        CGRect rect = r.rect;
...
- (BOOL)isStartGrabberContainsPoint:(CGPoint)point {
    ...
    if (CGRectIntersectsRect(startRect, endRect)) {
        CGFloat distStart = GGGTextCGPointGetDistanceToPoint(point, GGGTextCGRectGetCenter(startRect));
        CGFloat distEnd = GGGTextCGPointGetDistanceToPoint(point, GGGTextCGRectGetCenter(endRect));
...
- (BOOL)isSelectionRectsContainsPoint:(CGPoint)point {
    ...
    for (GGGTextSelectionRect *rect in _selectionRects) {
