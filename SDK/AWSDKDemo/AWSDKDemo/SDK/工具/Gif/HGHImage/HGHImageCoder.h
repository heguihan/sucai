...
/**
 Image file type.
 */
typedef NS_ENUM(NSUInteger, HHImageType) {
    HHImageTypeUnknown = 0, ///< unknown
    HHImageTypeJPEG,        ///< jpeg, jpg
    HHImageTypeJPEG2000,    ///< jp2
    HHImageTypeTIFF,        ///< tiff, tif
    HHImageTypeBMP,         ///< bmp
    HHImageTypeICO,         ///< ico
    HHImageTypeICNS,        ///< icns
    HHImageTypeGIF,         ///< gif
    HHImageTypePNG,         ///< png
    HHImageTypeWebP,        ///< webp
    HHImageTypeOther,       ///< other image format
};
 ...
 */
typedef NS_ENUM(NSUInteger, HHImageBlendOperation) {
 ...
 */
@interface HGHImageFrame : NSObject <NSCopying>
...
@property (nonatomic) HHImageDisposeMethod dispose;     ///< Frame dispose method.
@property (nonatomic) HHImageBlendOperation blend;      ///< Frame blend operation.
...
/**
 ...
 Example:
    ...
    HHImageDecoder *decoder = [HHImageDecoder decoderWithData:data scale:2.0];
    UIImage image = [decoder frameAtIndex:0 decodeForDisplay:YES].image;
 
    // Decode image during download:
    NSMutableData *data = [NSMutableData new];
    HHImageDecoder *decoder = [[HHImageDecoder alloc] initWithScale:2.0];
 ...
 */
@interface HGHImageDecoder : NSObject

@property (nullable, nonatomic, readonly) NSData *data;    ///< Image data.
@property (nonatomic, readonly) HHImageType type;          ///< Image data type.
 ...
 */
- (nullable HGHImageFrame *)frameAtIndex:(NSUInteger)index decodeForDisplay:(BOOL)decodeForDisplay;
...
/**
 ...
 @discussion It supports encoding single frame image with the type defined in HHImageType.
 It also supports encoding multi-frame image with GIF, APNG and WebP.
 
 Example:
    
    HHImageEncoder *jpegEncoder = [[HHImageEncoder alloc] initWithType:HHImageTypeJPEG];
    jpegEncoder.quality = 0.9;
    [jpegEncoder addImage:image duration:0];
    NSData jpegData = [jpegEncoder encode];
 
    HHImageEncoder *gifEncoder = [[HHImageEncoder alloc] initWithType:HHImageTypeGIF];
 ...
 */
@interface HGHImageEncoder : NSObject

@property (nonatomic, readonly) HHImageType type; ///< Image type.
 ...
 */
- (nullable instancetype)initWithType:(HHImageType)type NS_DESIGNATED_INITIALIZER;
...
/**
 ...
 @return The image data, or nil if an error occurs.
 */
+ (nullable NSData *)encodeImage:(UIImage *)image type:(HHImageType)type quality:(CGFloat)quality;
...
/**
 ...
 @return The image data, or nil if an error occurs.
 */
+ (nullable NSData *)encodeImageWithDecoder:(HGHImageDecoder *)decoder type:(HHImageType)type quality:(CGFloat)quality;
...
@interface UIImage (HGHImageCoder)
 ...
 */
@property (nonatomic) BOOL hh_isDecodedForDisplay;
 ...
 */
- (void)hh_saveToAlbumWithCompletionBlock:(nullable void(^)(NSURL * _Nullable assetURL, NSError * _Nullable error))completionBlock;
 ...
 */
- (nullable NSData *)hh_imageDataRepresentation;
...
CG_EXTERN HHImageType HHImageDetectType(CFDataRef data);

/// Convert HHImageType to UTI (such as kUTTypeJPEG).
CG_EXTERN CFStringRef _Nullable HHImageTypeToUTType(HHImageType type);

/// Convert UTI (such as kUTTypeJPEG) to HHImageType.
CG_EXTERN HHImageType HHImageTypeFromUTType(CFStringRef uti);

/// Get image type's file extension (such as @"jpg").
CG_EXTERN NSString *_Nullable HHImageTypeGetExtension(HHImageType type);



/// Returns the shared DeviceRGB color space.
CG_EXTERN CGColorSpaceRef HHCGColorSpaceGetDeviceRGB(void);

/// Returns the shared DeviceGray color space.
CG_EXTERN CGColorSpaceRef HHCGColorSpaceGetDeviceGray(void);

/// Returns whether a color space is DeviceRGB.
CG_EXTERN BOOL HHCGColorSpaceIsDeviceRGB(CGColorSpaceRef space);

/// Returns whether a color space is DeviceGray.
CG_EXTERN BOOL HHCGColorSpaceIsDeviceGray(CGColorSpaceRef space);



/// Convert EXIF orientation value to UIImageOrientation.
CG_EXTERN UIImageOrientation HHUIImageOrientationFromEXIFValue(NSInteger value);

/// Convert UIImageOrientation to EXIF orientation value.
CG_EXTERN NSInteger HHUIImageOrientationToEXIFValue(UIImageOrientation orientation);
 ...
 */
CG_EXTERN CGImageRef _Nullable HHCGImageCreateDecodedCopy(CGImageRef imageRef, BOOL decodeForDisplay);
 ...
 */
CG_EXTERN CGImageRef _Nullable HHCGImageCreateCopyWithOrientation(CGImageRef imageRef,
...
/**
 ...
 @return A new image, or NULL if an error occurs.
 */
CG_EXTERN CGImageRef _Nullable HHCGImageCreateAffineTransformCopy(CGImageRef imageRef,
 ...
 */
CG_EXTERN CFDataRef _Nullable HHCGImageCreateEncodedData(CGImageRef imageRef, HHImageType type, CGFloat quality);


/**
 Whether WebP is available in HHImage.
 */
CG_EXTERN BOOL HHImageWebPAvailable(void);
 ...
 */
CG_EXTERN NSUInteger HHImageGetWebPFrameCount(CFDataRef webpData);
 ...
 */
CG_EXTERN CGImageRef _Nullable HHCGImageCreateWithWebPData(CFDataRef webpData,
                                                           BOOL decodeForDisplay,
                                                           BOOL useThreads,
                                                           BOOL bypassFiltering,
                                                           BOOL noFancyUpsampling);

typedef NS_ENUM(NSUInteger, HHImagePreset) {
    HHImagePresetDefault = 0,  ///< default preset.
    HHImagePresetPicture,      ///< digital picture, like portrait, inner shot
    HHImagePresetPhoto,        ///< outdoor photograph, with natural lighting
    HHImagePresetDrawing,      ///< hand or line drawing, with high-contrast details
    HHImagePresetIcon,         ///< small-sized colorful images
    HHImagePresetText          ///< text-like
};
...
/**
 ...
 @param preset        Preset for different image type, default is HHImagePresetDefault.
 @return WebP data, or nil if an error occurs.
 */
CG_EXTERN CFDataRef _Nullable HHCGImageCreateEncodedWebPData(CGImageRef imageRef,
                                                             BOOL lossless,
                                                             CGFloat quality,
                                                             int compressLevel,
                                                             HHImagePreset preset);
