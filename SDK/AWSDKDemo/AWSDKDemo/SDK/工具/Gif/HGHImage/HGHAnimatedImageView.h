...
/**
 ...
 If the `image` or `highlightedImage` property adopt to the `HHAnimatedImage` protocol,
...
/**
 The HHAnimatedImage protocol declares the required methods for animated image
 display with HGHAnimatedImageView.
 
 Subclass a UIImage and implement this protocol, so that instances of that class 
 can be set to HGHAnimatedImageView.image or HGHAnimatedImageView.highlightedImage
 to display animation.
 
 See `HHImage` and `HHFrameImage` for example.
 */
@protocol HHAnimatedImage <NSObject>
@required
