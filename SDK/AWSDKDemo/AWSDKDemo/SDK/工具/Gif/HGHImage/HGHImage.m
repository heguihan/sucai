...
+ (HGHImage *)imageWithContentsOfFile:(NSString *)path {
    return [[self alloc] initWithContentsOfFile:path];
}

+ (HGHImage *)imageWithData:(NSData *)data {
    return [[self alloc] initWithData:data];
}

+ (HGHImage *)imageWithData:(NSData *)data scale:(CGFloat)scale {
    return [[self alloc] initWithData:data scale:scale];
}
...
- (instancetype)initWithData:(NSData *)data scale:(CGFloat)scale {
    ...
    @autoreleasepool {
        HGHImageDecoder *decoder = [HGHImageDecoder decoderWithData:data scale:scale];
        HGHImageFrame *frame = [decoder frameAtIndex:0 decodeForDisplay:YES];
        ...
        self.hh_isDecodedForDisplay = YES;
    }
    return self;
}
...
- (instancetype)initWithCoder:(NSCoder *)aDecoder {
    NSNumber *scale = [aDecoder decodeObjectForKey:@"HHImageScale"];
    NSData *data = [aDecoder decodeObjectForKey:@"HHImageData"];
    if (data.length) {
...
- (void)encodeWithCoder:(NSCoder *)aCoder {
    if (_decoder.data.length) {
        [aCoder encodeObject:@(self.scale) forKey:@"HHImageScale"];
        [aCoder encodeObject:_decoder.data forKey:@"HHImageData"];
    } else {
...
#pragma mark - protocol HHAnimatedImage
