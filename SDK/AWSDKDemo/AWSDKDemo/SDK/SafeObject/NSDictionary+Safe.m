...
-(instancetype)initWithObjects_st:(id *)objects forKeys:(id<NSCopying> *)keys count:(NSUInteger)count {
    ...
    for (NSUInteger i = 0; i < count; i++) {
        if (!(keys[i] && objects[i])) {
            NSString *toastStr= [NSString stringWithFormat:@"dict carsh key=%@, objcxx%@，index=%lu",keys[i],objects[i],i];
//            [AWTools makeToastWithText:toastStr];
            AWLog(@"dict carsh key=%@, objcxx%@，index=%lu",keys[i],objects[i],i);
            break;
        }else{
            rightCount++;
        }
